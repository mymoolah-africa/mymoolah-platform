...
class SmsService {
  constructor() {
    ...
    // Correct endpoint: https://rest.mymobileapi.com/bulkmessages
    this.apiUrl = process.env.MYMOBILEAPI_URL || 'https://rest.mymobileapi.com';
    this.apiPath = process.env.MYMOBILEAPI_PATH || '/bulkmessages';
  ...
  async sendUssdRegistrationSms(phoneNumber) {
    const msg = 'Welcome to MyMoolah! Your USSD wallet is ready. Dial *120*5616# to transact. Download the app for full features: mymoolah.africa';
  ...
  async sendUssdAirtimeSms(phoneNumber, amount, recipient) {
    const msg = `MyMoolah: Airtime R${amount} sent to ${recipient}. Dial *120*5616# for more.`;
  ...
  async sendUssdDataSms(phoneNumber, amount, recipient) {
    const msg = `MyMoolah: Data R${amount} sent to ${recipient}. Dial *120*5616# for more.`;
  ...
  async sendUssdSendMoneySms(phoneNumber, amount, recipientName) {
    const msg = `MyMoolah: R${amount} sent to ${recipientName}. Dial *120*5616# for balance.`;
  ...
  async sendUssdReceiveMoneySms(phoneNumber, amount, senderName) {
    const msg = `MyMoolah: You received R${amount} from ${senderName}. Dial *120*5616# for balance.`;
