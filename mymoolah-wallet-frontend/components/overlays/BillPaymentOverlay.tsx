...
/** Known biller stems - if both names contain same stem, they match (e.g. DSTV Now vs DSTV / Multichoice) */
const BILLER_STEMS = ['dstv', 'multichoice', 'pepkor', 'pep', 'netflix', 'showmax', 'oudtshoorn', 'municipality', 'pay@'];

/** Match biller names - overlay may show "DSTV Now Subscription" while seeded data has "DSTV / Multichoice Bill Payment" */
function billerMatches(beneficiaryBillerName: string | undefined, selectedBillerName: string): boolean {
  ...
  for (const stem of BILLER_STEMS) {
    if (stem.length >= 3 && a.includes(stem) && b.includes(stem)) return true;
  }
  return a.length >= 4 && b.length >= 4 && (a.includes(b.slice(0, 4)) || b.includes(a.slice(0, 4)));
}
