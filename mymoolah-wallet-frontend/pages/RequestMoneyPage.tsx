  ...
  const handleRequestMoney = async () => {
    ...
    try {
      ...
      if (formData.accountType === 'mymoolah') {
        ...
        const resp = await fetch(`${api}/api/v1/peach/payshap/rtp`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${token}`,
          ...
          body: JSON.stringify({
            amount: amount,
            currency: 'ZAR',
            creditorPhone: normalizeSAMobile(formData.payerMobileNumber),
            creditorAccountNumber: formData.payerAccountNumber,
            bankCode: getBankCode(formData.payerBankName || ''),
            bankName: formData.payerBankName,
            description: formData.reference || `PayShap RTP request from ${user.name}`,
          }),
        });
  ...
  const getNotificationMessage = () => {
    if (requestMode === 'recurring') {
      ...
      if (formData.accountType === 'mymoolah') {
        ...
        return `A payment request will be submitted to ${formData.payerBankName} for processing.`;
      }
    }
  };
        ...
        <>
          ...
          <div style={{ 
            ...
            <Dialog open={showDialog} onOpenChange={setShowDialog}>
              ...
              <DialogContent aria-describedby={undefined} style={{
                fontFamily: 'Montserrat, sans-serif',
                maxWidth: '340px',
                borderRadius: '16px'
              }}>
