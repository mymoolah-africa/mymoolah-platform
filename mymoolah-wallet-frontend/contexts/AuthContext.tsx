...
// Central mapping: normalize backend user payload into our frontend User shape
function mapBackendUserToContextUser(raw: any): User {
  // Prefer explicit phoneNumber; fall back to legacy keys
  const phone = normalizeSAMobileNumber(
    raw?.phoneNumber || raw?.phone || raw?.identifier || ''
  );
  return {
    id: String(raw?.id ?? ''),
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const changePassword = async (currentPassword: string, newPassword: string, confirmNewPassword: string) => {
    ...
    const response = await fetch(`${APP_CONFIG.API.baseUrl}/api/v1/auth/change-password`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Authorization': `Bearer ${token}`
