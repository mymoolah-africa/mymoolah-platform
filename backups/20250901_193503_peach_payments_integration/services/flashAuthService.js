...
class FlashAuthService {
     ...
     */
    validateCredentials() {
        const isLiveIntegration = process.env.FLASH_LIVE_INTEGRATION === 'true';
        
        if (isLiveIntegration && (!this.consumerKey || !this.consumerSecret)) {
            console.error('❌ Flash Auth Service: Missing FLASH_CONSUMER_KEY or FLASH_CONSUMER_SECRET environment variables');
            throw new Error('Flash API credentials not configured for live integration');
        }
        
        if (!isLiveIntegration) {
            console.log('ℹ️  Flash Auth Service: Operating in database mode - using seeded product catalogs');
        }
    }
