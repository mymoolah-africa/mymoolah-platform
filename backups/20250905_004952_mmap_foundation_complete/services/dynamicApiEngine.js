...
class DynamicApiEngine {
    constructor() {
        ...
        // Start automatic sync only if live integrations are enabled
        const hasLiveIntegrations = process.env.SUPPLIER_LIVE_INTEGRATIONS === 'true';
        if (hasLiveIntegrations) {
            this.startAutoSync();
        } else {
            console.log('ℹ️  Dynamic API Engine: Live integrations disabled - using database product catalogs');
        }
    }
