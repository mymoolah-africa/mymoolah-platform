...
# Usage: ./scripts/reset-production-password.sh
#
# This script will:
...
# 4. Verify the update
#
# Use when: "password authentication failed for user mymoolah_app"
# (Secret Manager and Cloud SQL password are out of sync)
##############################################################################
...
INSTANCE_NAME="mmtp-pg-production"
DB_USER="mymoolah_app"
SECRET_NAME="db-mmtp-pg-production-password"

echo "🔐 Reset Production Database Password"
echo "======================================"
echo ""
echo "This script will:"
echo "  1. Generate a new secure password"
echo "  2. Update password in Cloud SQL (${INSTANCE_NAME})"
echo "  3. Update password in Secret Manager (${SECRET_NAME})"
echo ""
echo -e "${YELLOW}⚠️  PRODUCTION: Ensure no services are using the DB during reset!${NC}"
echo ""
...
echo ""
echo -e "${GREEN}✅ Production password reset completed!${NC}"
echo ""
echo "📋 Next steps:"
echo "   1. Ensure proxy is running: ./scripts/ensure-proxies-running.sh"
echo "   2. Run migrations: ./scripts/run-migrations-master.sh production"
echo ""
