...
const initialKnowledgeBase = [
  ...
  // 3.2c – EFT deposit step-by-step
  {
    ...
    answer: 'To deposit money via EFT (Electronic Funds Transfer) from any South African bank: Step 1: Log in to your bank app or internet banking. Step 2: Add a new beneficiary or make a once-off payment with these details — Bank: Standard Bank, Account name: MyMoolah Treasury, Account number: 272406481, Branch code: 002154 (or use universal branch code 051001). Step 3: Enter the amount you want to deposit. Step 4: CRITICAL — In the "Reference" or "Recipient reference" field, enter your 10-digit mobile number (e.g. 0825571055). This is how the system identifies which wallet to credit. If you do not enter your mobile number as the reference, your deposit will be held in a suspense account until it can be manually allocated. Step 5: Submit the payment. EFT deposits typically reflect within 1 business day (same-day if submitted before your bank\'s cut-off time, otherwise next business day). For instant deposits, use PayShap instead.',
  ...
  // 3.2d – PayShap deposit step-by-step
  {
    ...
    answer: 'PayShap is the fastest way to deposit money into your MyMoolah wallet — it is processed in real-time (within seconds). Step 1: Open your bank app (Discovery Bank, FNB, Nedbank, Standard Bank, Absa, Capitec, or any PayShap-enabled bank). Step 2: Go to Pay or Transfer and select PayShap. Step 3: Choose "Pay ShapID". Step 4: Enter the ShapID: Mymoolah@STANDARDBANK (case-insensitive). The system will confirm the recipient as "MYMOOLAH (PTY) LTD". Step 5: Enter the amount. Step 6: In the "Recipient reference" field, enter your 10-digit mobile number (e.g. 0825571055). This is essential so the deposit is matched to your wallet. Step 7: Confirm and pay. Your wallet will be credited within seconds. PayShap is available 24/7 including weekends and public holidays. There is no delay — unlike EFT which takes up to 1 business day, PayShap deposits are instant.',
  ...
  // 3.2e – Why is my mobile number important as reference?
  {
    ...
    answer: 'Your 10-digit mobile number (e.g. 0825571055) is your wallet account identifier. When you make an EFT or PayShap deposit to the MyMoolah Treasury account, the system uses the payment reference to automatically match the deposit to your wallet and credit your balance. If you forget to include your mobile number, or use an incorrect reference, the deposit will be held in a suspense account and will need to be manually allocated by the MyMoolah support team. This may cause a delay. Always double-check that your reference is your correct 10-digit mobile number before submitting the payment.',
