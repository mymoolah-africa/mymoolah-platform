...
async function main() {
  ...
  try {
    ...
    console.log(`   Event ID:         ${result.eventId ?? '(not returned)'}`);
    console.log(`   Routed network:   ${result.network ?? '(not returned)'}`);
    console.log(`   Cost:             ${result.cost != null ? result.cost : '(not returned)'}`);
    console.log(`   Remaining credit: ${result.remainingBalance != null ? result.remainingBalance : '(not returned)'}`);
    console.log(`   Phone:            ${result.phoneNumber}`);
    console.log(`   Sent at:          ${result.timestamp}`);
    console.log('');
    console.log('ℹ️  NOTE: Gateway acceptance ≠ delivery. Watch your phone for');
    console.log('   ~30 seconds. If the SMS does not arrive:');
    console.log('     • Capture the Event ID above.');
    console.log('     • Contact MyMobileAPI support and ask for the delivery');
    console.log('       status + failure reason for that eventId.');
    console.log('     • Typical causes: unregistered alphanumeric sender ID,');
    console.log('       carrier spam filter, number on MNO block list.');
    console.log('');
    process.exit(0);
