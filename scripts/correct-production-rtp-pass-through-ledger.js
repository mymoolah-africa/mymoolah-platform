...
/**
 * Correct historical PayShap RTP ledger entries where the SBSA fee was posted
 ...
 *   node scripts/correct-production-rtp-pass-through-ledger.js --production --apply
 */
...
if (ENV_ARG !== '--production' || (!DRY_RUN && !APPLY)) {
  console.error('Usage: node scripts/correct-production-rtp-pass-through-ledger.js --production --dry-run');
  console.error('   or: node scripts/correct-production-rtp-pass-through-ledger.js --production --apply');
  process.exit(1);
}
...
function correctionReference(entry) {
  return `CORR-RTP-PASS-${entry.id}`;
}

(async () => {
  console.log(`\nPayShap RTP pass-through correction (${DRY_RUN ? 'dry-run' : 'apply'})`);
  ...
  try {
    const accountsResult = await client.query(
      ...
      [[ACCOUNT_SBSA_COST, ACCOUNT_SBSA_CLEARING, ACCOUNT_VAT_CONTROL]]
    );
    ...
    const entriesResult = await client.query(`
      ...
      WHERE je.reference LIKE 'SBSA-RTP-%'
    ...
    const existingCorrections = await client.query(
      `SELECT reference FROM journal_entries WHERE reference LIKE 'CORR-RTP-PASS-%'`
    );
    ...
    for (const entry of entriesResult.rows) {
      ...
      if (correctionRefs.has(ref) || oldClearingCredit > 0 || oldSbsaCostCredit <= 0 || oldVatCredit <= 0) {
        skipped++;
        continue;
      }

      const sbsaFeeVatIncl = roundMoney(oldSbsaCostCredit + oldVatCredit);
      const correctionLines = [
        {
          accountId: accounts.get(ACCOUNT_SBSA_COST).id,
          dc: 'debit',
          amount: oldSbsaCostCredit,
          memo: `Reverse historical RTP SBSA fee cost credit from ${entry.reference}`,
        },
        {
          accountId: accounts.get(ACCOUNT_VAT_CONTROL).id,
          dc: 'debit',
          amount: oldVatCredit,
          memo: `Reverse historical RTP SBSA pass-through VAT from MMTP VAT control for ${entry.reference}`,
        },
        {
          accountId: accounts.get(ACCOUNT_SBSA_CLEARING).id,
          dc: 'credit',
          amount: sbsaFeeVatIncl,
          memo: `Reclassify SBSA RTP fee as pass-through payable for ${entry.reference}`,
        },
      ];

      eligible++;
      totals.sbsaCostDebit = roundMoney(totals.sbsaCostDebit + oldSbsaCostCredit);
      totals.vatDebit = roundMoney(totals.vatDebit + oldVatCredit);
      totals.sbsaClearingCredit = roundMoney(totals.sbsaClearingCredit + sbsaFeeVatIncl);

      if (DRY_RUN) {
        console.log(`DRY  ${ref}: DR ${ACCOUNT_SBSA_COST} ${money(oldSbsaCostCredit)}, DR ${ACCOUNT_VAT_CONTROL} ${money(oldVatCredit)}, CR ${ACCOUNT_SBSA_CLEARING} ${money(sbsaFeeVatIncl)}`);
        continue;
      }
      ...
      try {
        const je = await client.query(
          ...
          [ref, `Correct PayShap RTP SBSA fee pass-through treatment for ${entry.reference}`]
        );
        ...
        posted++;
        console.log(`POST ${ref}: reclassified ${money(sbsaFeeVatIncl)} to ${ACCOUNT_SBSA_CLEARING}`);
      } catch (error) {
    ...
    console.log(`Posted corrections:    ${posted}`);
