...
**Version**: 3.3.0 - Android Google Play readiness
**Status**: ✅ **PRODUCTION LIVE** ✅ **API api-mm.mymoolah.africa** ✅ **WALLET wallet.mymoolah.africa** ✅ **ANDROID FOUNDATION BUILT** ✅ **GOOGLE PLAY BRANDING READY** ⏳ **PLAY ORGANISATION D-U-N-S PENDING** ✅ **EASYPAY V5 CASH-IN ONLY KB EMBEDDED** ✅ **WITHDRAW CASH PARTNER KB** ✅ **VAT PASS-THROUGH POLICY FORMALISED** ✅ **PRODUCTION RTP/RPP CORRECTIONS APPLIED** ✅ **PRODUCTION DB MIGRATED** ✅ **SBSA H2H EFT UAT ACTIVATION** ✅ **PAYSHAP RPP INSTANT PAYMENT** ✅ **CHART OF ACCOUNTS** ✅ **ELECTRICITY SUPPLIER COMPARISON** ✅ **LEDGER AUDIT** ✅ **CLOUD SCHEDULER** ✅ **CLOUD BUILD DEPLOYS** ✅ **LANGCHAIN RAG AI** ✅ **POPIA ENCRYPTION** ✅ **PAYSHAP RTP** ✅ **11 LANGUAGES** ✅ **MOJALOOP COMPLIANT**

---

## 🎯 **CURRENT STATUS OVERVIEW**

Production live at api-mm.mymoolah.africa and wallet.mymoolah.africa. **May 16, 2026:** Android Google Play readiness is in progress. The repo has Capacitor Android package `africa.mymoolah.wallet`, store-facing app name `mymoolah`, launcher/splash/Play icon generated from `mymoolah-wallet-frontend/assets/logo3.svg`, and release AAB tooling. Play Console organisation registration is paused until MyMoolah (Pty) Ltd's D-U-N-S number and matching D&B legal profile are confirmed; do not create a personal Play Console account without explicit André approval. **May 14, 2026:** EasyPay partner-alignment documentation pack: canonical **V5 callback URLs** (`api-mm` production, `staging.mymoolah.africa` for tests) and **payer-pays-fee-at-POS roadmap** (wallet credits full face value; `Amount` stays face-value cents) captured in `docs/integrations/EasyPay_API_Integration_Guide.md` v1.2.0, `docs/API_DOCUMENTATION.md`, finalisation plan Task 2b, handover, changelog, and session log `docs/session_logs/2026-05-14_1000_easypay-partner-comms-docs-sync.md`. No code or FAQ changes yet. **May 13, 2026:** EasyPay V5 cash-in-only support KB correction completed across UAT, Staging, and Production. `docs/FAQ_MASTER.md`, KB generator seed context, and `services/ragService.js` scope wording now distinguish EasyPay V5 as add-money / cash-in only and keep wallet withdrawals under approved Withdraw Cash partners. Environment KB results: UAT `333` active embedded entries, Staging `297`, and Production `297`, all with 0 embedding failures. Staging/Production required Cloud SQL proxy restarts because old fixed-token proxy processes were still listening but returning `read ECONNRESET`. **April 26, 2026:** VAT accounting strategy formalised in `docs/VAT_ACCOUNTING_STRATEGY.md`: MMTP VAT control/TaxTransaction rows are only for MMTP-owned revenue, markup, and commission; supplier, bank, client, and merchant throughput fees stay VAT-inclusive in clearing/payable accounts. Forward code was aligned for PayShap RPP/RTP, Zapper QR, historical EasyPay/Flash cash-out fee posting, wallet-bank EFT, and VAS commission. Production historical RPP/RTP corrections were applied non-destructively with `CORR-RPP-PASS-*` and `CORR-RTP-PASS-*` journals. Website SEO/content/FAQ/AI support will be managed in the separate website project/Claude Code; this MMTP repo remains responsible for secure APIs, MMAP integration, auth, audit, wallet, and backend services. Earlier ledger audit, `v_best_offers`, KYC raw SQL — see `CHANGELOG.md`.

### **📋 Latest: Android Google Play readiness (v3.3.0, May 16, 2026)** ⏳ **D-U-N-S BLOCKER**
Android foundation and Play branding are prepared in-repo. Package ID is `africa.mymoolah.wallet`; app name is `mymoolah`; Play icon uses `mymoolah-wallet-frontend/assets/logo3.svg`. External blocker: organisation Play Console registration needs MyMoolah (Pty) Ltd's D-U-N-S number and matching D&B profile before a banking/finance app should proceed. Session log: `docs/session_logs/2026-05-16_1038_google-play-duns-blocker.md`.

### **📋 Previous: EasyPay V5 partner comms documentation (v3.2.3, May 14, 2026)** ✅ **DOCS ONLY**
Canonical integration guide and API doc now list **production/staging V5 base URLs** and document **current** net-after-R6.33 wallet behaviour plus a **partner-gated roadmap** to **payer-paid POS fee** and **full face-value wallet credit**. Session log: `docs/session_logs/2026-05-14_1000_easypay-partner-comms-docs-sync.md`.

### **📋 Previous: EasyPay V5 cash-in-only support KB (v3.2.2, May 13, 2026)** ✅ **UAT + STAGING + PRODUCTION EMBEDDED**
Canonical support wording now treats EasyPay V5 as cash-in only and points wallet withdrawals to approved Withdraw Cash partners. UAT, Staging, and Production `ai_knowledge_base` rows and embeddings were refreshed with 0 embedding failures. Session log: `docs/session_logs/2026-05-13_0518_kb-refresh-all-envs.md`.

### **📋 Previous: VAT pass-through accounting strategy (v3.0.1, Apr 26, 2026)** ✅ **IMPLEMENTED / PRODUCTION CORRECTIONS APPLIED**
Canonical VAT strategy is live in documentation and code. PayShap RTP no longer posts SBSA pass-through fees to MMTP VAT control. Zapper QR, historical EasyPay/Flash cash-out fee posting, and supported cash-in/withdrawal fees reserve VAT control for MMTP-owned fee/margin only. Session log: `docs/session_logs/2026-04-26_1610_vat-pass-through-strategy.md`.

### **📋 Previous: Wallet-to-bank EFT H2H activation (v3.0.0, Apr 25, 2026)** ✅ **IMPLEMENTED / UAT TESTING PENDING**
Consumer bank payments now default to H2H EFT and offer Instant Payment via PayShap RPP. New wallet-bank quote/submit API, DB fee policies, payment tracking, settlement estimates, and Pain.002 reversal/refund handling are committed to `main`. Codespaces UAT and staging migrations are confirmed; next step is backend/frontend restart and wallet UI E2E testing. Session log: `docs/session_logs/2026-04-25_1121_wallet-bank-eft-h2h-activation.md`.
