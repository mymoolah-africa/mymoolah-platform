...
You were right: we were connecting with the wrong username. I was using MYMOOLAH (the filename prefix and our ISO-20022 party identifier) instead of mymoolahuser (the SFTP login user). I have re-run the full round-trip from our gateway at 34.35.137.166 and everything on the transport layer now works.
...
For context, on 2026-03-30 during the 13:xx SAST slot, Melanie's responses arrived very quickly: NACKs within about a minute, and the full ACK + INTAUDTST + FINAUDTST cycle inside five minutes. We are now well past that window, which suggests something on the processor side. Transport is clearly fine — /Outbox emptied as expected — so I do not believe the file is stuck on our side.

Could your H2H team please trace the file on your side and confirm three things: first, that MYMOOLAH_OWN11_Pain001v3_ZAR_TST_20260417110153RM1.xml was ingested from the SFTP processor into your Pain.001 inbound pipeline; second, whether today's TEST processing queue is paused or backlogged; and third, whether the mymoolahuser account is routed to the same processor as Melanie's earlier test account, or to a different queue.

I will keep polling /BAS on our side and will let you know the moment the response lands.

Where things stand overall: H2H SFTP connectivity is live on both TEST and PROD. Outbound Pain.001 disbursements are confirmed end-to-end on TEST at the transport layer, awaiting your processor ACK to close the functional loop. Inbound ACK / NACK / FINAUDTST / MT940 / MT942 infrastructure is ready on our side and our pollers will pick everything up automatically once the first TEST response closes.
