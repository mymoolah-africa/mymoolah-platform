...
| EasyPay V5 fee still netted from wallet; payer-pays-fee model pending partner | `controllers/easyPayController.js`, `services/easyPayDepositService.js`, `docs/integrations/EasyPay_API_Integration_Guide.md` §12.2 | Low/Medium — product/commercial misalignment until EasyPay confirms POS can collect fee while V5 `Amount` stays face value; ledger/recon must match settlement after change. | After written EasyPay confirmation: implement full face-value credit, remove wallet fee debit for channel fee, update JEs and `FAQ_MASTER` / KB pipelines; see `docs/session_logs/2026-05-14_1000_easypay-partner-comms-docs-sync.md` |
...
| OTT Nedbank payout not enabled on MyMoolah OTT account | `services/ott/ottPayoutService.js`, `routes/ott.js`, `docs/session_logs/2026-05-07_1133_ott-staging-payout-validation.md` | Medium — finance-approved terms now exist, but the controlled staging submit for provider `10` was rejected by OTT with `Provider is not authorised on this account :MyMoolah (Pty) Ltd`; MMTP reversed/refunded safely, but customer-facing Nedbank cannot go live until partner enablement is confirmed | Ask OTT to enable Nedbank Cardless Withdrawal for the MyMoolah account, then rerun one controlled staging R50 + fee test and verify payout/journal state before production enablement. |
| OTT Standard Bank cash payout not approved | `mymoolah-wallet-frontend/components/overlays/withdraw-cash/WithdrawCashOverlay.tsx`, `scripts/audit-ott-production-catalog.js`, `docs/integrations/OTT_MOBILE_INTEGRATION_FRAMEWORK.md` | Medium — DB/audit history may contain Standard Bank provider `2`, but André confirmed Standard Bank must approve the service before MyMoolah can expose it | Keep Standard Bank hidden from wallet payout surfaces and governance candidate lists until approval is received and documented. |
| OTT provider discovery does not verify complete fee terms before exposure | `routes/ott.js`, `services/ott/ottCommercialTermsService.js`, `services/ott/ottAuthorizedProviderPolicy.js` | Medium — an authorised provider code with an active but incomplete `supplier_commercial_terms` row can be returned by discovery and only fail later at quote time | Add a quoteable-provider filter that requires active complete fixed-fee terms before `/api/v1/ott/providers` or `/api/v1/ott/provider-limits` returns a cash-send provider. |
| Cloud SQL proxy health check only verifies listening port | `scripts/ensure-proxies-running.sh`, `scripts/db-connection-helper.js`, `docs/DATABASE_CONNECTION_GUIDE.md` | Low/Medium — long Codespaces sessions can leave a Cloud SQL Auth Proxy process listening with an expired fixed access token, causing repeated `read ECONNRESET` even though the script reports the proxy running | Enhance proxy management to probe `SELECT 1` per environment or restart proxies older than the token lifetime; until then, kill/restart the affected port and run a DB probe before Staging/Production KB or migration work. |
