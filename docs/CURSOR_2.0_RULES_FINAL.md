...
| PayShap TRF statement fallback safety layer not implemented | `services/standardbank/sbsaStatementService.js`, `services/standardbankDepositNotificationService.js`, future migration/table | Medium — missed PayShap/RTP/RPP credit notifications would not currently be auto-recovered from H2H statement `TRF` lines; loosening the current `TRF` skip would risk duplicate wallet credits | Build a separate delayed fallback engine with database-enforced unique bank-event fingerprints, ACID wallet/journal posting, notification-to-statement matching, suspense for ambiguous lines, and reconciliation reporting. Never auto-credit all `TRF` statement lines directly. |
