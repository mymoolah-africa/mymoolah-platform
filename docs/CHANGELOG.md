...
### Changes
- Added a shared wallet environment helper that treats Codespaces/local Vite dev, an explicit `VITE_NODE_ENV=uat` / related UAT build env, or a UAT API base URL as UAT. Staging/Production builds remain excluded.
...
### Summary
Aligned Staging OTT catalogue exposure to Jaco Snyders' active-provider spreadsheet/email source of truth, with ABSA CashSend provider `67` added from the email correction.

### Changes
- Added `config/ott-authorized-providers.json` as the checked-in email baseline and `services/ott/ottAuthorizedProviderPolicy.js` as the central policy helper.
...
- Added an `isGiftCard=true|false` filter to `/api/v1/overlay/vouchers/catalog` and wired the wallet to request the correct catalog side for each page.
...
- `npx jest tests/voucherCatalogBrandService.test.js --runInBand` passed 40/40.
...
- André later approved production catalog application after staging verification. `node scripts/ott-sync-providers.js --production --confirm-production --import-catalog` succeeded: 16 providers read, 1 limits row read, 16 providers upserted, and 4 catalog products imported in production (`OTT-156`, `OTT-157`, `OTT-68`, `OTT-69`). Post-sync production audit confirmed only ABSA CashSend, Nando's, Dis-Chem, Pick n Pay, and Shoprite / Checkers remain customer-facing; Standard Bank, PayShap, Amazon, Takealot, OTT Voucher, and AnyTime are hidden.
- Production OTT float accuracy verified read-only: ledger account `1200-10-08` balance is R1,000.00 and `supplier_floats.currentBalance` is R1,000.00, with `minimumBalance` R100.00.
- Production Cloud Run remains transaction-safe: `OTT_PAYOUT_ENABLED=false`, `OTT_LIVE_INTEGRATION=false`, and OTT base URL remains the OTT test endpoint until explicit live enablement. No production wallet debit, payout, voucher purchase, or live OTT transaction was performed.
...
- `npx jest tests/ott-product-purchase-service.test.js tests/voucherCatalogBrandService.test.js tests/ott-payout-service.test.js --runInBand --forceExit` passed 27/27; hotfix reruns `npx jest tests/ott-product-purchase-service.test.js --runInBand --forceExit` passed 5/5 and 6/6.
- `npm run build` in `mymoolah-wallet-frontend` passed.
- Cursor lints on touched files: no linter errors.
...
### Summary
Stopped hidden automatic PBAC retry creation after proxy-based RTP system rejects, then routed Discovery RTPs as PBAC/account-based from initiation when Discovery account details are supplied. Future bank-origin deposits also display with neutral wording and inbound bank-transfer iconography instead of PayShap/EFT-specific labels or wallet-transfer icons.
...
- Updated Discovery RTP initiation to use PBAC/account-based routing as the primary route when both mobile and account details are supplied, avoiding a Discovery proxy mandate that later rejects as not payable.
...
- Added `STANDARDBANK_RTP_DISCOVERY_PROXY_FIRST=true` as an emergency override to restore Discovery proxy-first routing if SBSA/Discovery require it later.
...
- The repair script loads `.env.codespaces` before the DB helper so Codespaces UAT admin credentials can be provided without changing tracked env templates.
...
- Codespaces UAT: `node scripts/repair-uat-transactions-reference.js --apply` followed by `./scripts/run-migrations-master.sh uat`; migrations `20260429_01_add_reference_to_transactions.js` and `20260429_02_create_ott_payouts.js` completed successfully.
...
### **Session Overview**
Discovered that Capitec RTP paid at 09:39 (R10.00 debited from Capitec) was never credited to the MyMoolah wallet. Root cause: SBSA batch callbacks use the UETR (UUID) as `orgnlMsgId`, but only the MMRTP msgId was stored in the DB. The UETR generated by `pain013Builder` was being discarded. Fixed by storing UETR in `requestId` and adding a fallback lookup. Standard Bank RTP confirmed end-to-end working (ACCC → wallet credited in 73ms).

### **Changes**
- **`services/standardbankRtpService.js`**:
  - Capture `uetr` from `buildPain013()` return value (was previously discarded)
  - Store `uetr` in `requestId` column (model always specified "UETR from Pain.013")
  - Keep `msgId` in `originalMessageId` (realtime callback path unchanged)
  - Add fallback lookup in `processRtpCallback`: if `originalMessageId` lookup fails, try `requestId` (catches SBSA batch callbacks that use UETR as orgnlMsgId)
  - Same fix applied to `retryRtpAsPbac` PBAC retry flow
