...
**Last Updated**: 2026-04-27 20:12 SAST
**Latest Feature**: **Figma retired from active frontend workflow — ✅ IMPLEMENTED / CODE-FIRST POSITIONING** — Active code and docs were swept so Figma is positioned correctly as historical only. MyMoolah frontend work is now explicitly code-first: React/TypeScript/Tailwind files and the MyMoolah design system are the source of truth. `ImageWithFallback` was moved from `components/figma/` to `components/common/`, imports were updated, obsolete Figma integration scripts/docs were removed or replaced with historical notes, and active docs/rules now state that Figma is not a current design platform, source of truth, approval gate, or read-only constraint. Archived/session-log references remain historical records only.
**Previous Feature**: **Agent skills accuracy and performance upgrade — ✅ IMPLEMENTED / DOCS-ONLY** — `.agents/skills/` was reviewed and upgraded for accuracy, lower token load, and fewer stale instructions. The two oversized skills now load quickly: `auditing` v2.2.0 is a concise ledger/VAT/reconciliation router with the old full content preserved in `auditing/reference-full.md`; `admin-portal-builder` v1.1.0 is a concise portal/RBAC/maker-checker guide with the old full content preserved in `admin-portal-builder/reference-full.md`. Correctness drift was fixed across active skills: JWT HS512, code-as-frontend-source-of-truth, MyMoolah brand green `#86BE41` + blue `#2D8CCA` + Montserrat, DB-helper-only direct scripts, safe concurrent-index migration example, Cloud Scheduler-first scheduled jobs, single-Redis lock wording, and visual skill routing via `_shared/visual-output-reference.md`. No runtime app code or database records changed.
**Older Feature**: **Production audit logic update + TXN#33 ledger correction — ✅ IMPLEMENTED / PRODUCTION AUDIT RECONCILES** — `scripts/production-full-audit.js` now matches the Apr 2026 VAT/pass-through policy: inclusive VAT rounding is handled correctly, RTP and RPP are audited separately, SBSA pass-through fees are validated through supplier clearing, and historical `CORR-RPP-PASS-*` / `CORR-RTP-PASS-*` correction journals are included in business-event checks. Production R50 wallet-ledger discrepancy traced to TXN#33 (`TXN-1775385125460-oiwqno`) where the VAS commission journal existed but the R50 face-value journal was missing. With André's explicit approval, `scripts/correct-production-missing-vas-face-txn33.js` dry-run was verified and then applied one non-destructive balanced journal: DR `2100-01-01` R50 / CR `1200-10-05` R50, reference `VAS-FACE-TXN-1775385125460-oiwqno`. Final production audit: 0 failures, 2 warnings only (R400 statistical anomaly; empty audit hash-chain table).
...
**Session logs**: `docs/session_logs/2026-04-27_2012_figma-retired-code-first-frontend.md` (this session — Figma retired from active workflow); `docs/session_logs/2026-04-27_1952_agent-skills-upgrade.md`; `docs/session_logs/2026-04-27_1530_production-audit-txn33-ledger-correction.md`; `docs/session_logs/2026-04-26_1610_vat-pass-through-strategy.md`; `docs/session_logs/2026-04-25_1121_wallet-bank-eft-h2h-activation.md`
