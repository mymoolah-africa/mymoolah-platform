...
**Last Updated**: 2026-04-21 12:00 SAST  
**Latest Feature**: **SBSA H2H RM5v2 over-limit re-run on TEST (v2.99.2)** — Re-ran Scenario 5 with R500,001 transaction (R1 over confirmed Cr Transaction Limit of R500,000). Result: ACK (RCVD, +18s), INTAUDTST (RJCT, +25s, Status Code **0009** "RUN EXCEEDS LIMIT", all 3 txns RJCT). No FINAUDTST emitted (consistent with RM9/RM10 — SBSA does not issue FINAUD after INTAUD RJCT). **Key finding**: 0006 per-tx over-limit path is unreachable on current TEST profile because Cr Transaction Limit == Sub Batch Limit (both R500,000). Any file with a single tx > R500,000 trips the batch limit (0009) before the per-tx check (0006) fires. **Additional finding**: Status Code 0009 is dual-purpose — used for both "invalid ordering account" (RM9) and "RUN EXCEEDS LIMIT" (RM5v2), differentiated by `AddtlInf` description only. Artefacts: `docs/test/sbsa-uat-rm5v2-responses-2026-04-20/`, report: `docs/test/sbsa-sftp-uat-rm5v2-report-2026-04-20.txt`, email draft: `docs/test/COLETTE_REPLY_2026-04-20_rm5-rerun.md`. Test harness: `scripts/test-sbsa-rm5-rerun.js`. Email to Colette proposes Option A (lower per-tx limit) or Option B (accept 0009 as operational reality) and requests PROD smoke test scheduling.  
**Previous Feature**: **Zapper SFTP user provisioned on sftp-1-vm (v2.99.1)** — SFTP user, firewall, GCS config, go-live email to Dillon.  
**Earlier Feature**: **EasyPay UAT test PINs generated + notification fix (v2.98.2)** — Test PINs sent to Razeen, SMS replaced with in-app notifications for EasyPay deposits.  
**Document Version**: 2.99.2  
**Session logs**: `docs/session_logs/2026-04-21_1200_sbsa-h2h-rm5v2-overlimit-rerun.md` (this session); prior `docs/session_logs/2026-04-20_1500_easypay-test-pins-and-notification-fix.md`; `docs/session_logs/2026-04-20_1144_sbsa-cash-withdrawal-policy-and-ringfence.md`  
