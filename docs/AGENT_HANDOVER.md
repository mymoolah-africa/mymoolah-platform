...
**Latest Documentation**: **FAQ audience split and self-service expansion — CUSTOMER-SAFE / WEBSITE IMPORT READY / KB REFRESH PENDING** — `docs/FAQ_MASTER.md` now has an audience legend for public Website, authenticated Wallet, Both, and Support KB entries. The FAQ was expanded for self-service coverage because MyMoolah does not rely on a traditional call-centre model: registration, Android access safety, KYC photo guidance, add-money via bank transfer and EasyPay V5, EasyPay pending/reference states, Send Money EFT/PayShap/request-money flows, beneficiaries, MoolahMove availability wording, VAS how-to steps, vouchers/gift cards, OTT/ABSA/Nedbank/eeziCash withdrawal troubleshooting, transaction history, notifications, fees, OTP/password safety, fraud/lost-phone support, developer/API access, and policy access. No runtime code, database, migrations, production data, or environment KB embeddings changed. Run `npm run check:kb:fresh` after edits; when André approves website/AI refresh, run the existing `generate:kb:faq:update` and `embed:kb` flow for the target environment.

**Last Updated**: 2026-05-16 10:38 SAST
**Latest Play Store Status**: **Google Play organisation registration pending — D-U-N-S REQUIRED / DO NOT CREATE PERSONAL ACCOUNT** — André started the Google Play Console organisation-account path for publishing `mymoolah`, but D&B lookup redirected to a U.S.-only form and the South African D-U-N-S number is not available yet. André will call/confirm the D-U-N-S details on Monday. Keep the app as an organisation publisher flow for MyMoolah (Pty) Ltd; do not switch to a personal Play Console account unless André explicitly approves that policy/compliance trade-off. Repository-side readiness is current: Android package ID `africa.mymoolah.wallet`, app/store name `mymoolah`, launcher/splash/Play icon generated from `mymoolah-wallet-frontend/assets/logo3.svg`, debug APK path documented, and release AAB tooling available. Remaining external blockers: active D-U-N-S number, matching Google payments profile legal name/address, Play Console organisation verification, signed AAB/upload-key custody, Play App Signing, Data Safety/financial declarations, screenshots/feature graphic, internal/closed testing, and Google review. Session log: `docs/session_logs/2026-05-16_1038_google-play-duns-blocker.md`.
**Latest Validation**: **Android release tooling reviewed — WALLET BUILD + KB GUARD PASSED / LOCAL AAB BLOCKED BY JAVA 24** — Continued from the interrupted release-tooling handover and reviewed the uncommitted Android build/docs changes. Wallet `npm run build` passed with the existing large chunk warning, root `npm run check:kb:fresh` passed, and Cursor lints on touched release-tooling files reported no errors. `mymoolah-wallet-frontend/docs/ANDROID_BUILD.md` was corrected so `sdkmanager --licenses` is run manually instead of piping `yes`, matching the rule not to accept Google/Android licenses on André's behalf. Follow-up doc cleanup removed the duplicate detailed Play visibility checklist from `ANDROID_BUILD.md`, points Play compliance/store readiness back to `docs/MOBILE_STORE_DEPLOYMENT_PLAN.md`, and explicitly warns not to place Android signing secrets, keystores, Google service account JSON, or Play API credentials in local `.env` files. Local `npm run build:android:release-aab` reached Gradle but failed with `Unsupported class file major version 68` because the local shell uses Java 24 and no local JDK 17 was discoverable via `/usr/libexec/java_home -V`; retry this in Codespaces or another JDK 17 shell before confirming the AAB path. Session logs: `docs/session_logs/2026-05-16_0710_android-release-tooling-validation.md`, `docs/session_logs/2026-05-16_0714_android-doc-dedupe-warning.md`.
**Latest Implementation**: **Android banking-grade foundation started — CAPACITOR ANDROID GENERATED / DEBUG APK BUILDS IN CODESPACES** — Began implementing the Google Play banking-grade wallet path. `mymoolah-wallet-frontend/android/` now exists with package/application ID `africa.mymoolah.wallet`; store-facing app name is `mymoolah`; `capacitor.config.ts` sets HTTPS scheme, cleartext false, mixed content false, and release WebView debugging false. Android launcher/splash resources and the generated 512x512 Play listing icon now use approved source `mymoolah-wallet-frontend/assets/logo3.svg`; Play icon path: `mymoolah-wallet-frontend/assets/google-play/mymoolah-play-icon-512.png`. Added Android scripts (`build:android:staging`, `build:android:production`, `build:android:debug-apk`, `build:android:release-aab`, `android:apk:path`, `android:aab:path`, `cap:sync:android`, `cap:open:android`) and Capacitor 6.2.1 dependencies aligned to current Node 18. `utils/authToken.ts` now supports native secure storage via `capacitor-secure-storage-plugin`; `AuthContext.tsx`, feedback, and KYC status flows no longer write/read auth tokens directly from legacy localStorage keys. Android manifest disables backup/data extraction and cleartext traffic, and `network_security_config.xml` contains current SPKI pins for `api-mm.mymoolah.africa` and `staging.mymoolah.africa`. Codespaces validation passed after André installed JDK 17 and Android SDK packages: wallet `npm run build:android:production`, `npx cap sync android`, and Android `./gradlew assembleDebug`. Debug APK path: `mymoolah-wallet-frontend/android/app/build/outputs/apk/debug/app-debug.apk`. Added `mymoolah-wallet-frontend/docs/ANDROID_BUILD.md` for repeatable setup/build/install guidance and Google Play visibility checklist. Release signing hook reads protected env vars (`MYMOOLAH_ANDROID_KEYSTORE_PATH`, `MYMOOLAH_ANDROID_KEYSTORE_PASSWORD`, `MYMOOLAH_ANDROID_KEY_ALIAS`, `MYMOOLAH_ANDROID_KEY_PASSWORD`) when CI/Secret Manager supplies them; no signing secrets are committed. Next: download/install debug APK on a physical device, create Play Console app `africa.mymoolah.wallet`, wire CI signing, add backup pin rotation, Play Integrity/root-risk, final screenshots/feature graphic, and real-device QA. Session log: `docs/session_logs/2026-05-15_2031_android-foundation.md`.
**Latest Operation**: **Staging wallet user cleanup — ✅ USER ID 12 DELETED / VERIFIED / PRODUCTION UNTOUCHED** — André requested deletion of all data for Staging wallet user ID `12`. Read-only dry-run confirmed the target was the USSD test account and found direct related rows: 1 wallet, 4 transactions, 30 EasyPay bills, and 2 payments. After explicit confirmation, the cleanup ran in one Staging transaction using `scripts/db-connection-helper.js` / `getStagingClient()`: deleted 2 `payments`, 30 `bills`, 4 `transactions`, 1 `wallets` row, and 1 `users` row. Verification returned zero direct rows for user ID `12` across `users`, `wallets`, `transactions`, `bills`, `payments`, `UserSettings`, `kyc`, `vouchers`, `otp_verifications`, and `user_referral_stats`. Remaining Staging wallet users are IDs `1`, `25`, and `26`. No UAT, Production, runtime code, migration, or deploy changed. Session log: `docs/session_logs/2026-05-15_1205_staging-user12-cleanup.md`.
**Latest Planning**: **Banking-grade Android deployment plan — DOCUMENTATION ONLY / NO CODE CHANGED** — Expanded `docs/MOBILE_STORE_DEPLOYMENT_PLAN.md` into the canonical Android-first Google Play deployment blueprint. The plan recommends Capacitor Android for production, rejects a simple TWA/WebView wrapper for banking-grade release, and defines security/performance gates for native secure storage, JWT/auth policy reconciliation, TLS/pinning, WebView hardening, POPIA-safe caching, low-end Android performance, Play Data Safety, release signing, and closed testing. No application code, database, production data, migrations, secrets, or runtime configuration changed. Session log: `docs/session_logs/2026-05-15_2026_android-deployment-plan.md`.
**Latest Feature**: **UAT-only wallet availability unfogging — ✅ WALLET BUILD PASSED / STAGING+PRODUCTION PROTECTED** — Added a shared wallet UAT detector and used it to remove fogging only in UAT, including the Codespaces `npm run dev` UAT testing flow where Vite exposes `DEV` and the API URL may be localhost. `mymoolah-wallet-frontend/components/overlays/withdraw-cash/WithdrawCashOverlay.tsx` now treats ABSA and Nedbank cards as available in UAT, including live OTT codes `67` and `4` plus legacy/test codes `112` and `10`; Staging/Production still use API availability as before. `mymoolah-wallet-frontend/components/overlays/shared/AddAccountModal.tsx` enables the MoolahMove tab and international account fields in UAT only, now with fully rounded modal corners and correct `mobile_money` / `international_bank` service payloads. `mymoolah-wallet-frontend/pages/SendMoneyPage.tsx` removes the MoolahMove fog/Coming Soon pill in UAT only and now wires Add New Beneficiary MoolahMove to the country/payment-method/account form instead of the temporary info modal. Validation passed: wallet `npx tsc --noEmit`, wallet `npm run build`, and Cursor lints on touched frontend files. Session log: `docs/session_logs/2026-05-14_1525_uat-wallet-unfogging.md`.
**Previous Feature**: **EasyPay channel correction — ✅ UAT+STAGING+PRODUCTION KB EMBEDDED** — Applied Razeen Abrahams' correction that MyMoolah's EasyPay Receiver ID is not activated on excluded eCommerce / non-in-person channels because of fraud risk. `docs/FAQ_MASTER.md` now guides users to pay EasyPay cash-in references in person at participating tills/kiosks, excludes Plusmore, Vodacom-related channels or wallets such as VodaPay, WalletDoc, Prepaid24, EFTCorp / Ukheshe, banking-app bill-payment routes such as Nedbank app flows, and Clicks website/app payments, and names only the direct bank integrations Razeen confirmed: Absa, Tyme Bank, Capitec, and Old Mutual Bank, subject to onboarding and availability. KB refresh completed in all three environments: UAT updated 133 FAQ rows and embedded 337 active rows; Staging inserted 4 / updated 129 and embedded 301 active rows after restarting stale proxy `6544`; Production inserted 4 / updated 129 and embedded 301 active rows after restarting stale proxy `6545`. Readback verification confirmed the corrected EasyPay question is active and embedded in UAT, Staging, and Production, includes the eCommerce exclusion wording, and names only the direct banks Razeen confirmed. Session log: `docs/session_logs/2026-05-14_1435_easypay-channel-correction-kb.md`.
**Previous Feature**: **EasyPay hosted SFTP automation — ✅ IMPLEMENTED / TESTS PASS / DEPLOYMENT GATED** — Implemented a cloud-side EasyPay-hosted SFTP pull bridge so daily EasyPay SOF transaction files can be collected without WinSCP or a local Mac. `services/reconciliation/EasyPaySftpPullService.js` connects to EasyPay SFTP using Secret Manager-bound env vars, matches `easy%.%` files, validates SOF structure with `EasyPayAdapter`, skips files already in GCS, and uploads valid files to `gs://mymoolah-sftp-inbound/easypay/` for the existing `SFTPWatcherService` + `ReconciliationOrchestrator` path. `routes/reconciliation.js` adds `POST /api/v1/reconciliation/scheduled-easypay-sftp-pull`, Cloud Scheduler-authenticated, disabled by default via `EASYPAY_SFTP_PULL_ENABLED=false`, and scoped to Staging/Production only so UAT/local skips even if the flag is accidentally set. `scripts/setup-cloud-scheduler.sh` only creates the scheduler if `EASYPAY_SFTP_PULL_CREATE_SCHEDULER=true`. `scripts/deploy-backend.sh` now optionally binds `easypay-sftp-host/port/username/password-{staging|production}` secrets, and docs/env templates describe the Staging -> Production rollout without storing credentials. Validation passed: `node --check services/reconciliation/EasyPaySftpPullService.js routes/reconciliation.js`, `bash -n scripts/deploy-backend.sh scripts/setup-cloud-scheduler.sh`, and focused Jest 15/15. No live EasyPay SFTP connection, scheduler creation, deployment, production write, or credential commit was performed. Session log: `docs/session_logs/2026-05-14_1425_easypay-hosted-sftp-automation.md`.
**Previous Feature**: **EasyPay retailer network KB update — ✅ UAT KB EMBEDDED / STAGING+PRODUCTION PENDING ANDRE TEST** — Added EasyPay's integration-confirmation information to `docs/FAQ_MASTER.md` §9b without storing any SFTP password or raw credential detail. The FAQ now covers EasyPay add-money retailer/channel networks (Shoprite/Checkers/USave, Spar-family stores, Game/Makro/Builders Warehouse, Lewis, Woolworths, Clicks, Plusmore/PEP, VodaPay, Walletdoc, UniPay, Prepaid24, EFTCorp, major banks, Kazang, EasyPay Money Market, RA Cellular, Cell Air, Powertime, TopIt Up, plus PEP/Ackermans and Pick n Pay/Boxer expected after channel testing), SalesSupport escalation for receiver queries, and customer-safe WinSCP/SFTP clarification for macOS operations. UAT KB refresh completed: initial `generate:kb:faq:update` hit `read ECONNRESET`; restarted UAT proxy on port `6543`, verified DB `SELECT NOW()`, then reran `generate:kb:faq:update` (4 inserted, 129 updated), `activate:kb` (4 activated), and `embed:kb` (337 active embedded, 0 failed). Staging and Production were intentionally not updated until André tests UAT. Session log: `docs/session_logs/2026-05-14_1336_easypay-retailer-kb-uat.md`.
**Previous Feature**: **MoolahMove coming-soon treatment — ✅ WALLET BUILD PASSED** — Fogged out MoolahMove entry points in the wallet Send Money / Add Account UI. `mymoolah-wallet-frontend/components/overlays/shared/AddAccountModal.tsx` now disables the MoolahMove tab, adds a blue `Coming Soon` pill, and keeps the international account form gated behind the availability flag so users cannot add MoolahMove accounts while the service is unavailable. `mymoolah-wallet-frontend/pages/SendMoneyPage.tsx` now aligns the existing disabled MoolahMove tiles to the same greyed blue-pill Coming Soon style. Validation passed: wallet `npx tsc --noEmit`, wallet `npm run build`, and Cursor lints on touched frontend files. Session log: `docs/session_logs/2026-05-14_1108_moolahmove-coming-soon.md`.
**Previous Feature**: **OTT cash withdrawal result + history fix — ✅ BACKEND TESTS + WALLET BUILD PASSED** — Investigated André's live R50 Nedbank OTT withdrawal where the SMS arrived but the wallet did not show the expected success state and Transaction History did not show a voucher/PIN. Root cause for the success state was an API contract mismatch: submit returned top-level `status`, but poll returned status only under `updateResult`, while `WithdrawCashOverlay.tsx` checked `result.status`. `services/ott/ottPayoutService.js` now returns a top-level safe payout shape from `pollPayoutStatus`; `apiService.ts` normalizes submit/poll results; and `WithdrawCashOverlay.tsx` treats failed/cancelled/reversed result bodies as errors instead of success. Read-only Production diagnostic for payout `OTT-1778740719890-c098bf26` confirmed the payout and wallet transaction were completed, provider code `4`, but the persisted provider response did not contain PIN/voucher/token/serial/code fields, so the historical R50 row cannot display a credential from API data. Future OTT responses are now allowlist-scanned for credential fields before redaction; encrypted credential values are stored only when field encryption is configured, masked credential metadata is added for transaction history, and `TransactionHistoryPage.tsx` / `TransactionDetailModal.tsx` show masked card text plus reveal/copy in details only when the API actually has a credential. Validation passed: `node --check services/ott/ottPayoutService.js controllers/walletController.js`, `npx jest tests/ott-payout-service.test.js --runInBand --forceExit` 17/17, `npx jest tests/wallet-ott-display.test.js --runInBand --forceExit` 3/3, wallet `npx tsc --noEmit`, wallet `npm run build`, and Cursor lints on touched files. Session log: `docs/session_logs/2026-05-14_0940_ott-cashout-status-history.md`.
**Previous Feature**: **Notification drawer freshness + EasyPay notification hardening — ✅ WALLET BUILD + EASYPAY TESTS PASSED** — Investigated stale/lagging wallet notifications. Root cause found: EasyPay deposit notification creation used provider source `easypay`, but the notifications table/model source enum only accepts `system`, `admin`, or `job`, so notification creation failed after the wallet credit. `services/notificationService.js` now normalizes unsupported `type`, `severity`, `category`, and `source` values before insert, preventing best-effort notification writes from failing due caller-specific labels. `mymoolah-wallet-frontend/contexts/MoolahContext.tsx` now fetches recent notifications with `status=all` for the bell drawer and still derives the red indicator/blocking prompt from unread rows only, so old unread items no longer make the drawer look stale. Validation passed: `node --check services/notificationService.js controllers/easyPayController.js`, `npx tsc --noEmit`, wallet `npm run build`, EasyPay Jest 10/10, and Cursor lints on touched files. Session log: `docs/session_logs/2026-05-14_0145_notification-drawer-freshness.md`.
**Previous Feature**: **Launch Add Money card readiness — ✅ WALLET BUILD PASSED** — Prepared the wallet Add Money service list for launch while Flash voucher top-up is on hold. `mymoolah-wallet-frontend/pages/TransactPage.tsx` now shows `EasyPay Top-up` directly under `Bank Transfer`; `Voucher Top-up` is moved below EasyPay and masked as `Coming Soon`, matching the disabled treatment used for unavailable cash-out services. No backend service logic, routes, migrations, or production data changed. `docs/FAQ_MASTER.md` ring-fenced balance wording was adjusted so Flash/1Voucher self-load channels are referenced only when available, avoiding support copy that implies the on-hold service is live. Validation passed: `npx tsc --noEmit`, `npm run build` in `mymoolah-wallet-frontend`, and Cursor lints on `TransactPage.tsx`. Session log: `docs/session_logs/2026-05-14_0130_launch-add-money-card-readiness.md`.
**Previous Feature**: **EasyPay cash top-up voucher lifecycle + voucher logo sweep — ✅ BACKEND TESTS + WALLET BUILD PASSED** — Completed the approved EasyPay Cash Top-up Voucher Lifecycle Plan without editing the plan file. `controllers/easyPayController.js` now marks the matching `easypay_topup` voucher as `redeemed` inside the successful V5 `paymentNotification` DB transaction, preserving duplicate callback idempotency and recording callback/payment metadata for audit/history; EasyPay fee rows now persist with the customer-facing description `Transaction fee`. Follow-up in `controllers/voucherController.js` reconciles legacy pending EasyPay top-up vouchers against already-paid `bills` in both the voucher listing response and voucher balance summary, so old paid top-ups leave `Mine`, appear in history, and no longer inflate the Home dashboard voucher count without requiring a DB migration or production row mutation. `mymoolah-wallet-frontend/pages/VouchersPage.tsx` now treats unpaid EasyPay top-up PINs as pending cash top-up payment instructions, excludes them from total spendable voucher value, shows completed top-ups as paid history rows rather than active `Mine` cards, and renders pending top-ups with a cleaner payment-instruction layout. `mymoolah-wallet-frontend/utils/transactionDisplay.ts` defensively renders all fee rows as `Transaction fee`, so historical EasyPay/VAT breakdown descriptions are not exposed in wallet cards or history. Follow-up removed `EasyPay Cash-out Voucher` from `Vouchers > Create`; EasyPay top-up stays under `Add Money > EasyPay Top-up`, and future cash-out belongs under `Withdraw Cash`. Follow-up logo import fix updated wallet references from `*-logo.png` to `*_logo.png`, then added `mymoolah-wallet-frontend/components/overlays/digital-vouchers/brandLogos.ts` as the shared real-logo resolver for voucher cards and purchase modals using the expanded `*_logo` asset inventory. Added focused coverage in `tests/easypay-v5-controller.test.js`. Validation passed: `node --check controllers/easyPayController.js controllers/voucherController.js tests/easypay-v5-controller.test.js`, EasyPay Jest 10/10, `npx tsc --noEmit` in the wallet frontend, `npm run build` in the wallet frontend, and Cursor lints on touched files. Targeted ESLint still reports pre-existing legacy debt in `VouchersPage.tsx`. Session log: `docs/session_logs/2026-05-13_2246_easypay-topup-voucher-lifecycle.md`.
**Previous Feature**: **EasyPay top-up PIN spacing — ✅ FRONTEND BUILD PASSED** — Removed spaces from EasyPay top-up PIN display and copy behavior only. `mymoolah-wallet-frontend/components/overlays/topup-easypay/TopupEasyPayOverlay.tsx` now displays and copies the newly created EasyPay top-up PIN as continuous digits. `mymoolah-wallet-frontend/pages/VouchersPage.tsx` now applies digit-only display/copy only to backend `easypay_topup` / `easypay_topup_active` records; non-top-up EasyPay voucher formatting, MMVoucher grouping, cash-out EasyPay screens, backend voucher data, and transaction-history descriptions were left unchanged. Validation passed: `npm run build` in `mymoolah-wallet-frontend`; Cursor lints on touched files reported no errors. Targeted ESLint on these legacy files still reports pre-existing unrelated lint debt. Session log: `docs/session_logs/2026-05-13_1157_easypay-topup-pin-spacing.md`.
**Previous Feature**: **EasyPay V5 — production retest hardening — ✅ CODE + TESTS PASS** — Hardened the Bill Payment Receiver V5 path before the next EasyPay production retest. `middleware/easypayAuth.js` now treats only `STAGING=true` as staging, so production Cloud Run deployments with `STAGING=false` no longer expose UAT/Bearer fallback behavior; EasyPay `SessionToken` handling remains unchanged. `controllers/easyPayController.js` now accepts missing optional `Reference` on `authorisationRequest` using the existing `NOREF` internal fallback, and `paymentNotification` still completes wallet + ledger work before responding but no longer waits for in-app notification creation before returning `{ EchoData }`. Added focused coverage in `tests/easypay-auth.test.js` and `tests/easypay-v5-controller.test.js`. Validation passed: `node --check middleware/easypayAuth.js controllers/easyPayController.js tests/easypay-v5-controller.test.js tests/easypay-auth.test.js`, `npx jest tests/easypay-v5-controller.test.js tests/easypay-auth.test.js --runInBand` (9/9), and Cursor lints on touched files. Session log: `docs/session_logs/2026-05-13_1016_easypay-v5-retest-hardening.md`.
**Previous Feature**: **EasyPay V5 — partner email drafts + doc sync — ✅ DOCUMENTATION + FAQ SOURCE** — Captured in-chat **draft outbound emails** to EasyPay: (1) **canonical callback URLs** for production (`https://api-mm.mymoolah.africa/billpayment/v1/...`) and staging (`https://staging.mymoolah.africa/billpayment/v1/...`), alias `/api/v1/easypay/*`, SessionToken auth, and **possible causes** of `authorisationRequest` timeouts (client/LB idle timeout, cold start, URL drift); (2) **payer-pays-fee at POS** so the **wallet credits full PIN face value** (preferred contract: V5 `Amount` remains **integer cents = face value**; fee collected in EasyPay retail layer — avoids new V5 fields). Updated **`docs/integrations/EasyPay_API_Integration_Guide.md` v1.2.0** (§12.1–12.2), **`docs/API_DOCUMENTATION.md`**, **`docs/EASYPAY_V5_AGENT_HANDOVER.md`**, **`docs/EASYPAY_V5_FINALISATION_PLAN.md`** (Task 2b roadmap), **`docs/PROJECT_STATUS.md`**, **`docs/README.md`**, **`docs/CHANGELOG.md`**, **`docs/CURSOR_2.0_RULES_FINAL.md`** (tech debt), **`docs/FAQ_MASTER.md`** §9b (EasyPay cash-in fee Q&A; `_Last updated` 14 May 2026). **No application or database code changes.** Run approved **`generate:kb:faq:update*` / `embed:kb*`** when André wants embeddings refreshed after this FAQ change. Session log: `docs/session_logs/2026-05-14_1000_easypay-partner-comms-docs-sync.md`.

**Previous Feature**: **Support KB — EasyPay V5 cash-in only — ✅ UAT + STAGING + PRODUCTION DB REFRESH COMPLETE** — Removed support copy that described EasyPay as a wallet cash-out: updated `docs/FAQ_MASTER.md` §9b (PEP, retail chains, where to get cash), replaced the EasyPay cash-out GPT gap-fill topic in `scripts/generate-knowledge-base.js`, fixed fee/platform overview seed strings, updated `services/ragService.js` strict scope line, `docs/WITHDRAWALS_COMPLIANCE_AND_KB.md` intents, `docs/CURSOR_2.0_RULES_FINAL.md`, `docs/CHANGELOG.md` (2026-05-13), `docs/AI_SUPPORT_SYSTEM.md` v3.2.1. `npm run check:kb:fresh` passed. KB database refresh is complete: UAT `generate:kb:faq:update` updated 129 rows and `embed:kb` embedded 333 active rows with 0 failures; Staging `generate:kb:faq:update:staging` inserted 4, updated 125, and `embed:kb:staging` embedded 297 active rows with 0 failures; Production `generate:kb:faq:update:production` inserted 4, updated 125, and `embed:kb:production` embedded 297 active rows with 0 failures. Staging/Production required killing stale Cloud SQL proxy processes on ports `6544`/`6545` before rerunning because `ensure-proxies-running.sh` only detects a listening process, not expired fixed-token proxy sessions. Do not rerun the KB scripts unless the source FAQ changes again. Deploy backend if `services/ragService.js` strict-scope code has not yet been deployed to the target environment; allow the RAG cache TTL to expire before support-chat retests.

**Previous Feature**: **Staging OTT live cashout provider fee terms — ✅ MIGRATION READY / STAGING ONLY / PRODUCTION UNTOUCHED** — Investigated Staging wallet console errors on `/api/v1/ott/payouts/quote` after André deployed backend image `20260512_v1`. Cloud Run logs showed the backend was failing before any OTT HTTP call with `OTT_FEE_POLICY_INCOMPLETE`: `fixedFeeExVat` was missing for live provider codes `67` (ABSA CashSend) and `4` (Nedbank Cardless Withdrawal). Read-only DB comparison confirmed Staging had active `supplier_commercial_terms` rows for `4` and `67` with null `fixed_fee_ex_vat`, `mmtp_fee_ex_vat`, and `reversal_fee_ex_vat`, while the approved fixed R13 cashout fee structure is already established. Added `migrations/20260512_01_seed_ott_live_absa_nedbank_payout_terms.js`, an idempotent migration that updates active `OTT` / `cash_send` / `fixed_fee` rows for provider `4` and `67` to `fixed_fee_ex_vat=9.96`, `mmtp_fee_ex_vat=1.34`, `reversal_fee_ex_vat=9.96`, `fixed_fee_vat_rate=0.15`, customer fee incl VAT `13.00`, and customer-facing true; it inserts rows only if an environment has no active row. Validation passed: `node --check` and Cursor lints on the migration. Production DB was not touched; André will run `./scripts/run-migrations-master.sh staging 20260512_01_seed_ott_live_absa_nedbank_payout_terms` and redeploy/retest Staging only.
**Latest Feature**: **OTT-Mobile withdrawal network KB update — ✅ UAT+STAGING+PRODUCTION KB EMBEDDED** — André confirmed the new OTT-Mobile cash-withdrawal support information: MyMoolah now has two customer-facing OTT-Mobile cash-withdrawal options in the support KB. Nedbank cash-withdrawal vouchers can be collected at Nedbank ATMs, Checkers, Checkers Hyper, Shoprite, Usave, House & Home, OK Foods, OK Furniture, Pick n Pay, Boxer, PEP, Ackermans, OTT outlets, Kazang outlets, and Flash shops. ABSA CashSend can be collected at ABSA ATMs, Pick n Pay, and Boxer. Updated `docs/FAQ_MASTER.md` §9 with customer-facing how-to guidance for starting Withdraw Cash in-app, reviewing fees and total debit, wallet debit before collection, OTP/security steps, safe PIN/reference handling, and support escalation if collection fails. Replaced the older support wording that said Nedbank was not customer-facing. Follow-up clarified merchant-facing terminology: customers must present the credential as a `Nedbank cash-withdrawal voucher` or `ABSA CashSend` / `ABSA cash-withdrawal voucher`, because outlet staff may not know MyMoolah and customers should not ask for a "MyMoolah withdrawal" at the outlet. Added `--faq-only` support to `scripts/generate-knowledge-base.js` plus `generate:kb:faq:update*` npm scripts for focused FAQ updates without extra GPT gap-fill generated rows. Initial UAT generation processed 207 entries (61 inserted, 146 updated), activated pending rows, and embedded 329 active entries with 0 failures. Staging generation processed 207 entries (40 inserted, 167 updated) after restarting stale Cloud SQL proxies and embedded 293 active entries with 0 failures. Production generation processed 207 entries (40 inserted, 167 updated) and embedded 293 active entries with 0 failures. Follow-up FAQ-only refresh for PEP/Ackermans and merchant-facing wording updated 127 FAQ rows and inserted 0 in each environment, then embedded UAT 329 active entries, Staging 293 active entries, and Production 293 active entries with 0 failures. Final DB verification: UAT `329` active / `329` active embedded / `0` inactive, last updated `2026-05-12T17:28:26.239Z`; Staging `293` active / `293` active embedded / `11` inactive, last updated `2026-05-12T18:30:00.376Z`; Production `293` active / `293` active embedded / `2` inactive, last updated `2026-05-12T18:33:19.057Z`. Active Nedbank/ABSA merchant-facing wording rows were verified in all three DBs.
**Previous Feature**: **AI support knowledgebase refresh and freshness guard — ✅ SOURCE UPDATED / LOCAL GUARD ADDED / UAT+STAGING+PRODUCTION KB EMBEDDED** — Swept the existing LangChain RAG support architecture and confirmed the maintained support KB source is `docs/FAQ_MASTER.md`, generated into `ai_knowledge_base` through `scripts/generate-knowledge-base.js` and embedded through `scripts/embed-knowledge-base.js`; no duplicate support service was added. Updated `docs/FAQ_MASTER.md` from its 20 April 2026 baseline with May support-facing changes: Gift Cards vs Buy Retail Vouchers, Gift Cards tile hiding when no approved catalog rows exist, voucher amount validation, current approved retail voucher examples, OTT cash-withdrawal availability/approval rules, and referral SMS troubleshooting. Added `scripts/check-support-kb-freshness.js`, `npm run check:kb:fresh`, `.cursor/hooks.json`, and Cursor rule/doc updates so commit/push shell commands are blocked when `FAQ_MASTER.md` is older than the newest changelog entry. Enhanced `scripts/generate-knowledge-base.js` with `--update-existing` plus `generate:kb:update` scripts so existing matching questions are refreshed rather than skipped. André approved UAT -> Staging -> Production generation/embedding. Final DB verification: UAT `268` active / `268` active embedded / `0` inactive, last updated `2026-05-12T16:35:02.651Z`; Staging `253` active / `253` active embedded / `11` inactive, last updated `2026-05-12T16:26:20.215Z`; Production `253` active / `253` active embedded / `2` inactive, last updated `2026-05-12T16:31:53.132Z`.
**Latest Feature**: **OneGate virtual card information stack — ✅ DOCUMENTED / HTML ARTEFACT / PDF-SAFE LAYOUT** — Created `docs/integrations/OneGate_Virtual_Card_Info_Stack.html`, a polished partner-facing information stack for OneGate covering the proposed MMwallet-linked pre-funded virtual Mastercard debit card and NFC POS merchant spend use case. The document positions MyMoolah as a South African banking-grade Treasury Platform and PASA TPPP-certified fintech platform with Standard Bank of South Africa as sponsor bank, explains the employer/company disbursement-to-wallet flow, MyMoolah prefunded OneGate float, OneGate/card-network POS authorisation path, wallet debit, and merchant settlement flow. It includes audit and settlement controls, short regulatory posture summaries, and strict phase-one exclusions for ATM cash-out, retail cash-back, cash withdrawal, cross-border use, and unapproved high-risk expansion. Layout was refined for A4 landscape PDF export with co-branded MyMoolah/OneGate header, flow visuals, print colour preservation, and Chrome `Background graphics` guidance. No runtime code, database schema, migrations, production data, secrets, ledger postings, or deployments changed. Session log: `docs/session_logs/2026-05-11_1832_onegate-info-stack.md`.
**Latest Feature**: **Production voucher governance enforcement — ✅ PRODUCTION DB APPLIED / BACKEND+WALLET DEPLOYED / READ-ONLY VERIFIED** — André approved fixing the wallet voucher catalog permanently by making Product Catalog Governance the Production source of truth while preserving the existing working retail voucher cards. Read-only sweeps confirmed the wallet overlay is API-driven and not using a static legacy voucher list; the live issue was the backend fallback recognizer path remaining active because `PRODUCT_CATALOG_GOVERNANCE_ENABLED` was not set. Added recognition for the approved additions (`OTT Voucher`, `NetFlorist`, `EasyBet`, `GBets`, `Gold Rush`), changed voucher catalog grouping so variable voucher groups purchase through the variable variant, and added denomination-specific purchase identities so fixed grouped cards submit the correct `productId`/`variantId`. Added `scripts/approve-production-voucher-governance.js`, a dry-run-first Production helper using `scripts/db-connection-helper.js`; dry-run showed 29 retail cards preserved, 93 approval rows, and 10 fallback/unknown rows still blocked. Production apply approved/published those 93 mappings, including MobileMart `OTT Variable Voucher` only (`5BOioLwx80GvyiwTch2U`, R5-R5,000), four NetFlorist SKUs, Flash EasyBet `1967`, Flash GBets `1970`, and Flash Gold Rush `1969`. OTT Variable minimum was updated from `1000` to `500` cents with matching constraints. Updated `scripts/deploy-backend.sh` so Production deploys set `PRODUCT_CATALOG_GOVERNANCE_ENABLED=true` (Staging default remains false unless overridden). Validation passed: syntax checks, `bash -n`, focused Jest 50/50, wallet `npm run build`, and Cursor lints. Production backend initially deployed revision `mymoolah-backend-production-00212-ltt` with governance flag true; Production wallet initially served revision `mymoolah-wallet-production-00052-6l5`. Follow-up fixed a visual regression where governance `icon_key` slugs like `amazon` and `gold-rush` were returned as display icons, causing oversized text in voucher cards; Production backend hotfix revision `mymoolah-backend-production-00215-j69` is serving 100% traffic. Follow-up wallet UI fix now validates variable voucher amounts immediately in `ProductDetailModal.tsx`, so out-of-range OTT entries such as R2 or R6,000 show inline errors and keep the purchase button disabled before submit; Production wallet revision `mymoolah-wallet-production-00053-jbc` serves 100% traffic. Read-only post-deploy checks confirmed backend `/health` OK, wallet HTTP 200, selected mappings approved/published, and OTT Variable min/max at `500`/`500000`. No wallet debit, voucher purchase, payout submit, destructive delete, or migration was run.
**Previous Feature**: **OTT Production authorised catalog rollout — ✅ PRODUCTION DB APPLIED / DEPLOYED / READ-ONLY VERIFIED** — Continued the approved OTT Production rollout without editing the plan file. Generalised `scripts/sync-ott-authorized-products.js` and `scripts/stage-ott-catalog-governance.js` for explicit `--production --confirm-production` dry-run/apply flows via `getProductionClient()`, kept dry-run as default, and retained non-destructive SQL only. Production dry-run initially exposed two policy risks, so the policy was fixed before apply: `config/ott-authorized-providers.json` now explicitly keeps Standard Bank Instant Money non-customer-facing until partner approval, and `services/ott/ottAuthorizedProviderPolicy.js` treats catalog `voucher` and `gift_card` types as compatible while keeping payout types separate. André selected the policy-fix/rerun path. Final Production apply results: `sync-ott-authorized-products.js --production --confirm-production --apply` hid 21 unsupported terms and deactivated 19 unsupported OTT products/variants; `stage-ott-catalog-governance.js --production --confirm-production --apply` approved/published mapping `1842` for `OTT-20` Shoprite / Checkers. Read-only Production audit confirmed one approved/published OTT voucher governance mapping, Standard Bank remains held/not customer-facing, and OTT float `1200-10-08` is R877.10. Wallet `TransactPage.tsx` now hides only the `Gift Cards` tile when `apiService.getVouchers(undefined, undefined, true)` returns zero, preserving `/gift-cards-overlay` and overlay code. Validation passed: node syntax checks, Cursor lints, focused Jest 67/67, and wallet `npm run build`. Commit `a9297357` was pushed to `origin/main`; Production backend deployed with image tag `20260511_ott_prod_rollout`, final serving revision `mymoolah-backend-production-00209-jcj`; Production wallet deployed with the same tag, serving revision `mymoolah-wallet-production-00051-9x4`. Post-deploy read-only checks passed after restarting the stale local Production Cloud SQL proxy: backend `/health` OK, wallet HTTP 200, `sync-ott-authorized-products.js --production --confirm-production` completed, and `audit-ott-production-catalog.js --production` confirmed the catalog state. Session log: `docs/session_logs/2026-05-11_1440_ott-production-rollout.md`.
**Previous Feature**: **OTT authorised product synchronization — ✅ IMPLEMENTED / STAGING APPLIED + DEPLOYED / PRODUCTION GATED** — Implemented the attached OTT Product Synchronization Plan without editing the plan file. Confirmed the existing pipeline already had OTT API sync/import, product catalog governance, and wallet exposure filters; added a central spreadsheet/email-based authorisation policy instead of creating a duplicate product service. `config/ott-authorized-providers.json` records Jaco's email baseline with ABSA CashSend provider `67`; `services/ott/ottAuthorizedProviderPolicy.js` parses the workbook when provided and is reused by OTT provider sync, payout route filtering, payout quote/submit approval, and Staging governance publication. Added `scripts/sync-ott-authorized-products.js` as a Staging-only dry-run/apply reconciliation tool using `scripts/db-connection-helper.js`; it parsed `~/Downloads/Payout Provider List.xlsx`, called OTT read-only discovery (16 providers, 1 limits row), compared spreadsheet/email/API/DB/products/governance, then applied non-destructive Staging changes: 21 unsupported terms hidden, 21 unsupported OTT products/variants deactivated, 2 unsupported mappings unpublished, and authorised mappings for `OTT-68`, `OTT-69`, and `OTT-20` published. Code commit `a04c4878` was pushed to `origin/main`, then Staging backend was deployed with image tag `20260511_ott_authorized_sync`; final serving revision `mymoolah-backend-staging-00548-hxv`. Focused validation passed: node syntax checks, Cursor lints, Jest 67/67, post-deploy `sync-ott-authorized-products.js --staging`, post-deploy `audit-ott-production-catalog.js --staging`, and Staging `/health`. No production writes, wallet debits, payout submissions, voucher purchases, migrations, or wallet frontend deployments were performed. Production rollout remains gated behind André approval after a production dry-run review. Session log: `docs/session_logs/2026-05-11_1209_ott-authorized-product-sync.md`.
...
MyMoolah Treasury Platform (MMTP) is South Africa's premier Mojaloop-compliant digital wallet and payment solution. It provides: wallet services, VAS (airtime, data, vouchers, bill payments, electricity), approved Withdraw Cash partners, EasyPay V5 cash-in (add money), referrals, KYC, and automated multi-supplier reconciliation. AI support powered by LangChain RAG v3.2 with environment-specific KB embeddings and topic filtering. Self-hosted security layer: Redis-backed distributed rate limiting, bot scoring, PoW CAPTCHA, AI cost gateway, GCP Cloud Armor WAF + Cloud CDN — no SaaS dependencies. **Production**: api-mm.mymoolah.africa, wallet.mymoolah.africa. Built on Node.js, PostgreSQL, React, GCP. For operating rules, workflow, and constraints, read `docs/CURSOR_2.0_RULES_FINAL.md` first.
