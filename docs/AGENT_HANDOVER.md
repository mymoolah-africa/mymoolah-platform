...
**Last Updated**: 2026-04-27 15:30 SAST  
...
**Previous Feature**: **VAT strategy formalisation and pass-through correction — ✅ IMPLEMENTED / PRODUCTION CORRECTIONS APPLIED / PUSHED TO MAIN** — Canonical policy added in `docs/VAT_ACCOUNTING_STRATEGY.md`: MMTP records VAT control/TaxTransaction rows only for MMTP-owned revenue, markup, and commission. Supplier/bank/client/merchant throughput fees are VAT-inclusive pass-through amounts posted to clearing/payable accounts. Forward code aligned for PayShap RPP/RTP, Zapper QR, EasyPay cash-out, and Flash cash-out. Production historical cleanup: RPP was already corrected with `CORR-RPP-PASS-*`; RTP correction script `scripts/correct-production-rtp-pass-through-ledger.js` applied 6 `CORR-RTP-PASS-*` journals, reclassifying R34.50 from SBSA cost/VAT control to supplier clearing. Final read-only checks found no remaining Zapper VAT debit lines, EasyPay provider-expense lines, or Flash cash-out VAT-control lines matching legacy pass-through patterns. Commits pushed to `main`: `e4c89c5c` (handover/session log) and `1b279e25` (VAT/code/docs).  
...
**Session logs**: `docs/session_logs/2026-04-27_1530_production-audit-txn33-ledger-correction.md` (this session — audit script VAT/pass-through alignment and TXN#33 R50 production ledger correction); `docs/session_logs/2026-04-26_1610_vat-pass-through-strategy.md`; `docs/session_logs/2026-04-25_1121_wallet-bank-eft-h2h-activation.md`; `docs/session_logs/2026-04-24_0900_sbsa-penny1-success-and-penny2-plan.md`  
