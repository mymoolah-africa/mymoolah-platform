...
**Last Updated**: 2026-04-25 11:21 SAST  
**Latest Feature**: **Wallet-to-bank EFT H2H activation (v3.0.0, commit `f288790f`)** — Bank payments in `SendMoneyPage.tsx` now default to SBSA H2H EFT with an optional **Instant Payment** toggle that uses existing PayShap RPP rails. Added database-backed effective-dated customer transaction fee policies for future MMAP control, seeded UAT EFT fee `R2.00`, added `wallet_bank_payments` tracking, quote/submit APIs at `/api/v1/wallet-bank-payments`, EFT receiver availability estimates using `15:00 SAST` cutoff + Saturday intake + SA public holidays, and Pain.002-driven rejection reversal/refund handling. **UAT next**: pull in Codespaces, run migration `20260425110000_create_wallet_bank_payments_and_fee_policies.js`, restart backend/frontend, test EFT default and Instant Payment toggle. **Production remains gated** until Penny #2 FINAUD and R10 inbound validation are complete. Session log: `docs/session_logs/2026-04-25_1121_wallet-bank-eft-h2h-activation.md`.  
**Previous Feature**: **SBSA H2H PROD Penny #2 — ✅ SUCCESS so far (ACK + INTAUD captured in 22 s; FINAUD expected Wed 2026-04-29 due to Freedom Day roll)** — Penny #2 uploaded to SBSA /Outbox at 2026-04-24 09:30:40 SAST (UTC 07:30:40.490Z), sftp exit 0, MD5 `71b795e738680cd0079060f2d292d008` verified laptop↔VM↔SBSA. **ACK RCVD at 09:30:58 (T+18 s), INTAUD PDNG / TxSts PDNG / 0000 at 09:31:02 (T+22 s)** — huge improvement over Penny #1's 60-min delay. `/Outbox` emptied within 20 s. All SBSA stated SLAs (pickup ≤1 min, ACK ≤1 min, INTAUD ≤5 min) outperformed by wide margin. SBSA gracefully rolled our `ReqdExctnDt=2026-04-27` to `2026-04-28` due to Freedom Day (27 Apr, SA public holiday) — correct behaviour on their side. FINAUD and settlement now expected **Wed 2026-04-29**. Evidence: `docs/test/sbsa-sftp-prod-penny2-report-2026-04-24.txt` (interim), `docs/test/sbsa-prod-penny2-responses-2026-04-24/` (ACK+INTAUD XMLs + baseline-diff poll log). Combined Penny #1+#2 success email delivered to André as chat copy-paste (for dispatch to Melanie+Colette). `SBSA_H2H_GO_LIVE=false` still in force — gate flips only after Penny #2 FINAUD confirmed Wednesday and both-statement settlement verified. **Pending inbound validation**: R10 deposit ref `0825571055` (User ID 1) from 2026-04-23 sits in SBSA MT940 file unprocessed — will auto-credit when gate flips and statement poller runs. André chose "wait for gate flip" over manual crediting — cleanest E2E validation of the MT940 → wallet-credit path on real production data.  
**Older Feature**: **SBSA H2H PROD Penny #1 — ✅ SUCCESS (root cause on SBSA side, resolved 2026-04-23 11:54 SAST)** — Full Production H2H round-trip confirmed end-to-end with real money. 2026-04-23 INCONCLUSIVE verdict at T+30min was caused by a **missing processing rule on SBSA's side** (per Melanie Block, SBSA, email 2026-04-23 18:45 SAST). SBSA fixed the rule at 11:54 SAST; picked up our Pain.001 at ~11:55; delivered **ACK RCVD** at 11:56:47, **INTAUD PDNG / 0000** at 11:56:51, and **FINAUD ACSP / 0000** overnight at 2026-04-24 05:56:06 SAST. Both statements confirm real-money settlement 2026-04-24: R1.00 debit on `272406481`, R1.00 credit on `10111730633`.  
**Even Older**: **SBSA H2H pain002 parser/poller correctness fixes + PROD Penny tooling (v2.99.6)** — Shipped 2026-04-23 (commit `e8005f90`). Deploy gate `SBSA_H2H_GO_LIVE`, AddtlInf capture, response-type classifier, widened filename filter, INTAUD-RJCT terminal logic, UNPAID-authoritative over FINAUD. 23 new unit tests. All 149 disbursement + 26 standardbank tests green.  
**Document Version**: 3.0.0  
**Session logs**: `docs/session_logs/2026-04-25_1121_wallet-bank-eft-h2h-activation.md` (this session — wallet-bank EFT activation); `docs/session_logs/2026-04-24_0900_sbsa-penny1-success-and-penny2-plan.md` (Penny #1 RESOLVED, Penny #2 SUCCESS interim, R10 inbound validation pending); `docs/session_logs/2026-04-23_1132_sbsa-h2h-prod-penny-run.md`; `docs/session_logs/2026-04-23_1023_sbsa-h2h-prod-penny.md`  
