...
**Last Updated**: 2026-04-22 14:15 SAST  
**Latest Feature**: **SMS diagnostic tooling + `eventId` parser bugfix + end-to-end UAT delivery verification (v2.99.5)** — Built `scripts/test-sms.js` as a permanent standalone SMS diagnostic that drives the production `services/smsService.js` code path (same auth, endpoint, payload). Fixed a silent response-parsing bug in `smsService.js` that had been masking the MyMobileAPI `eventId` as `undefined` in every production OTP/referral/USSD SMS log since go-live — code was looking for `.messages[0].id` but the real shape is top-level `{ cost, remainingBalance, eventId, costBreakdown:[{ network }], errorReport }`. Also removed a 300-char log truncation that was hiding the `errorReport.faults[]` payload. Delivery verified end-to-end: clean accept + screenshot-confirmed receipt on `+27784560585` (Leonie). **Root-cause closure** on the "OTP not received" symptom that triggered this session: reading the Dec 30, 2025 session log surfaced a pre-existing, documented carrier-level blacklist on André's primary number `+27825571055` (line 123–130 of `2025-12-30_1115_sms-integration-referral-testing-complete.md`) — never a code issue, just an open ticket that was never actioned with MyMobileAPI/SMS South Africa. **No migrations. No frontend rebuild. Backend restart required** only to pick up the improved `messageId` logging.  
**Previous Feature**: **Login regression fix — `idNumber` ciphertext-tolerant validator + targeted `.save()` calls (v2.99.4)** — Caught and fixed a **critical login regression** introduced by the v2.99.2 security hardening before it shipped. The new `User.resetLoginAttempts()`/`incrementLoginAttempts()` helpers and the `lastLoginAt` update all call `.save()` on the full User instance, which re-runs Sequelize validation on the encrypted-at-rest `idNumber` field. When `afterFind`'s `decrypt()` returns ciphertext unchanged (FIELD_ENCRYPTION_KEY missing/mismatched — e.g. Codespaces env drift), the instance carries `enc:v1:...` into validation and the plaintext `len [5,20]` rule fails with `ValidationError: Validation len on idNumber failed` → every successful login returns **500**. Applied defense-in-depth: (1) **Root-cause fix at model level**: replaced `idNumber.validate.len` with a custom `plaintextLength()` validator that skips length checks when the value starts with `enc:v1:` — protects **every** caller (admin panel, future code, tests), not just login. (2) **Controller belt-and-braces**: `login` (`lastLoginAt`), `changePassword`, `resetPassword`, `incrementLoginAttempts`, and `resetLoginAttempts` now all use targeted `{ fields: [...], validate: false, hooks: false }` so they never revalidate unrelated encrypted-at-rest fields. (3) **Break-glass tool**: shipped `scripts/reset-user-password.js` — env-aware (UAT/Staging/Prod with `--confirm-production` guard), audit-logged, bcrypt cost 12, atomic counter-clear.  
**Earlier Feature**: **Referral 3-level cleanup + timestamp field-mapping tech debt fix (v2.99.3)** — 14 Sequelize models migrated to explicit `createdAt`/`updatedAt` field mapping; stale 4-level references removed from `models/Referral.js` and `docs/DEVELOPMENT_GUIDE.md`.  
**Document Version**: 2.99.5  
**Session logs**: `docs/session_logs/2026-04-22_1415_sms-diagnostic-tooling-and-delivery-verification.md` (this session); prior `docs/session_logs/2026-04-22_1215_idnumber-validation-regression-fix.md`; `docs/session_logs/2026-04-22_1152_referral-3level-cleanup-and-timestamp-mapping-fix.md`  
...
**Session Status**: ✅ **COMPLETE** — SMS diagnostic tooling + `eventId` parser fix + UAT delivery verification (v2.99.5)  
**Last Session**: 2026-04-22 14:15 SAST — Built permanent SMS diagnostic tool, fixed silent `eventId` parser bug, confirmed end-to-end UAT delivery, surfaced pre-existing `+27825571055` blacklist.

### **Most Recent Work (2026-04-22 14:15)**
- **Built `scripts/test-sms.js`** — standalone CLI diagnostic using production `smsService.js` code path. Supports custom text, random-OTP simulation, referral templates with 11-language support. Exits with classified codes for CI.
- **Fixed `smsService.js` response parser**: MyMobileAPI returns `{ cost, remainingBalance, eventId, costBreakdown:[{ network }], errorReport }` — code was looking for `.messages[0].id`. Result: every production OTP/referral/USSD `messageId` log had been `undefined` since go-live. Now exposes real `eventId`, `network`, `cost`, `remainingBalance`.
- **Removed 300-char log truncation** on gateway responses — full `errorReport.faults[]` now captured for diagnostics.
- **Delivery verified**: Leonie `+27784560585` received test SMS (eventId `16810483661`), screenshot confirmed by user. HD `+27798569159` gateway-accepted (eventId `16810483645`).
- **Root-cause closure on "OTP not received"**: Dec 30, 2025 session log `2025-12-30_1115_sms-integration-referral-testing-complete.md` lines 123–130 documents `+27825571055` as carrier-level blacklisted with action "Contact SMS South Africa support to investigate blacklist" — never closed. NOT a code issue.
- **Commits**: `bec95903`, `d718f4a3`, `61e10fd7` — all pushed to `origin/main`.

### **Previous Work (2026-04-22 12:15)**
- **Login regression fix (v2.99.4)**: `idNumber` ciphertext-tolerant validator + targeted `.save()` calls in `User` model and `authController` (`login`, `changePassword`, `resetPassword`). Prevents 500 on every successful login when `FIELD_ENCRYPTION_KEY` drifts between envs. Shipped `scripts/reset-user-password.js` break-glass tool.

### **Previous Work (2026-04-11 12:00)**
...
| Date | Update |
|------|--------|
| Apr 22 (14:15) | **SMS Diagnostic Tooling + `eventId` Parser Fix + UAT Delivery Verification (v2.99.5)**: Built `scripts/test-sms.js` — permanent standalone CLI that exercises production `smsService.js` (supports custom text / `--otp` / `--referral --lang=XX`). Fixed silent `eventId` parsing bug in `services/smsService.js` — MyMobileAPI returns `{ cost, remainingBalance, eventId, costBreakdown:[{ network }], errorReport }` top-level, code was looking for `.messages[0].id`, so every production OTP/referral/USSD `messageId` has been `undefined` since go-live. Removed 300-char log truncation. Delivery confirmed: Leonie `+27784560585` received SMS (eventId `16810483661`, screenshot verified). Root-cause closure on "OTP not received to `+27825571055`": documented carrier-level blacklist from Dec 30, 2025 session log, never actioned with SMS South Africa support. **No migrations. Backend restart** only for improved messageId logging. Session log: `docs/session_logs/2026-04-22_1415_sms-diagnostic-tooling-and-delivery-verification.md` |
| Apr 22 (12:15) | **Login Regression Fix — `idNumber` Ciphertext-Tolerant Validator (v2.99.4)**: Caught critical 500-on-every-login bug before shipping. Custom `plaintextLength()` validator on `User.idNumber` skips length checks when value is `enc:v1:...` ciphertext (protects against `FIELD_ENCRYPTION_KEY` drift). Targeted `{ fields, validate:false, hooks:false }` on `login`/`changePassword`/`resetPassword`/`incrementLoginAttempts`/`resetLoginAttempts`. Shipped `scripts/reset-user-password.js` break-glass tool (env-aware, bcrypt cost 12, `--confirm-production` guard). Zero lint errors. No migrations. No frontend rebuild. Session log: `docs/session_logs/2026-04-22_1215_idnumber-validation-regression-fix.md` |
