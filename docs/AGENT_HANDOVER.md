...
**Last Updated**: 2026-04-23 11:32 SAST  
**Latest Feature**: **SBSA H2H PROD Penny #1 — EXECUTED BUT INCONCLUSIVE (v2.99.6 live run)** — Single R1.00 Pain.001 uploaded to SBSA PROD /Outbox at 2026-04-23 10:55:37 SAST from `sftp-1-vm` via manual `sftp -b` batch. Upload itself clean (exit 0, MD5 `e5f0cefb86fd09d9e130d3b6011d14ef` verified across laptop/VM/SBSA-reported-size). **ZERO pain.002 responses in 30-min polling window** (no ACK, NACK, INTAUD, FINAUD, UNP_DATA, or VET_DATA). File still visible on SBSA /Outbox at T+30min, untouched (mtime 10:55). Inbound channel confirmed healthy throughout — PROVSTMT files landed on normal 15-min cadence including 11:05 and 11:20 SAST. Per runbook rule, STOPPED and did NOT re-upload. Escalation email sent to Colette and Melanie requesting disposition check (has file been picked up, does MsgId appear in SBSA logs, is there a manual first-use release step). Extended 4-hour poller running on `sftp-1-vm` (PID 221588, 120s interval) to catch any late responses. `SBSA_H2H_GO_LIVE=false` remains in force — scheduler pollers, app-level upload, and recon watcher all OFF in PROD. MsgId `MM-PRODPENNY1776933922946-MOB8KTXF`, EndToEndId `PROD-PENNY-1776933922946-01`. Evidence: `docs/test/sbsa-sftp-prod-penny-report-2026-04-23.txt`, `docs/test/COLETTE_REPLY_2026-04-23_prod-penny.md`, `docs/test/sbsa-prod-penny-responses-2026-04-23/poll-log-T0-to-T30.log`. **AWAITING SBSA REPLY before any re-upload or further deployment.**  
**Previous Feature**: **SBSA H2H pain002 parser/poller correctness fixes + PROD Penny tooling (v2.99.6)** — Shipped 2026-04-23 (commit `e8005f90`). New `SBSA_H2H_GO_LIVE` deploy gate (default `false`), AddtlInf capture, response-type classifier, widened filename filter (ACK/NACK/INTAUD/FINAUD/UNP_DATA/VET_DATA), INTAUD-RJCT terminal logic, UNPAID-authoritative over FINAUD, filename propagation through parser. 23 new unit tests (9 parser + 2 poller + 6 service, plus 1 updated), all 149 disbursement + 26 standardbank tests green. Also: `fix(deploy): define log/err helpers before SBSA H2H gate block` (`449da4d1`) and `docs(sbsa-penny): --scp-flag=-P 2222` (`3e1a9ecf`).  
**Older Feature**: **SBSA H2H RM5v2 over-limit re-run on TEST (v2.99.2)** — Re-ran Scenario 5 with R500,001 transaction. Status Code 0009 "RUN EXCEEDS LIMIT", no FINAUDTST, AddtlInf is sole differentiator between 0009 "invalid account" and 0009 "RUN EXCEEDS LIMIT". See prior session logs.  
**Document Version**: 2.99.6  
**Session logs**: `docs/session_logs/2026-04-23_1132_sbsa-h2h-prod-penny-run.md` (this session — PROD Penny #1 live run); `docs/session_logs/2026-04-23_1023_sbsa-h2h-prod-penny.md` (v2.99.6 prep); prior `docs/session_logs/2026-04-21_1200_sbsa-h2h-rm5v2-overlimit-rerun.md`, `docs/session_logs/2026-04-20_1500_easypay-test-pins-and-notification-fix.md`  
