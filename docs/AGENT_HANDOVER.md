...
**Last Updated**: 2026-04-21 17:25 SAST  
**Latest Feature**: **Zapper SFTP user provisioned on sftp-1-vm (v2.99.1)** — Completed MyMoolah-side provisioning for Zapper's daily mark-off SFTP feed. Created `zapper` SFTP user on Thorntech gateway (`sftp-1-vm` 34.35.137.166) mirroring MobileMart precedent row-for-row: `users.id=5 home_folder_id=4`, `folder.id=4 absolute_path=/zapper cloud_connection_inherited_id=1 (→ gs://mymoolah-sftp-inbound/zapper/)`, `user_folder_permission` READ_WRITE, `authorities` ROLE_SFTP, `public_key.id=3 name=zapper-dillon-2026-04-21` with Dillon's RSA 2048 key (fingerprint `SHA256:EkGSJ40gqWvwpF+x1m30vB9t5duCBnU9xMUk8GA6Gl4`, cross-verified on Mac + VM). Firewall rule `allow-zapper-sftp` on tcp:5022 scoped to 52.213.37.176/32 (Zapper's AWS eu-west-1 NAT). Forward-only migration `20260421_01_fix_zapper_sftp_path.js` corrected `recon_supplier_configs.sftp_path` to `/home/zapper/inbox` on UAT/staging/production (original `20260413_01` was already applied with pre-fix value; banking-grade pattern — never edit an applied migration). GCS `zapper/` + `zapper/inbox/` placeholders in place. Go-live email drafted at `docs/integrations/ZAPPER_EMAIL_DILLON_GO_LIVE.md` with 1-row synthetic test CSV (`integrations/zapper/samples/zapper_markoff_TESTHANDSHAKE.csv`). **Pending**: Dillon's test upload (smoke test). **Temp open**: 102.164.83.33/32 on `allow-admin-ssh-2222-temp` (added because IPv6-broken Mac blocks gcloud IAP WebSocket; to be removed post smoke test). No runtime code changes.  
**Previous Feature**: **EasyPay UAT test PINs generated + notification fix (v2.98.2)** — Generated `easypay_test_pins.csv` (50 rows, 45 DB bills + 5 invalid PINs) and sent to Razeen with UAT SessionToken via onetimesecret.com. Fixed `easyPayController.js`: replaced SMS with in-app notification engine for deposit confirmations. Added PIN expiry alignment question (A5) to Q&A checklist. EasyPay scheduling UAT sprint.  
**Earlier Feature**: **SBSA H2H UAT six-scenario test executed on TEST (v2.98.1)** — Ran Colette's six UAT scenarios back-to-back against SBSA TEST (196.8.85.62:5022), 20-min intervals, 15:55 → 18:36 SAST. All six uploads accepted by /Outbox/ within 2s. Verdicts: RM7 PASS, RM8 PASS, RM9 PASS* (0009 vs expected 0003), RM10 PASS, RM11 PARTIAL (under limit), RM12 PARTIAL (VET absent). UNPAID can override FINAUD per-tx. Prior: MobileMart SFTP activation Phase 1; SBSA H2H SFTP hardening (v2.98.0).  
**Document Version**: 2.99.1  
**Session logs**: `docs/session_logs/2026-04-21_1725_zapper-sftp-user-provisioned.md` (this session); prior `docs/session_logs/2026-04-20_1530_easypay-test-pins-sent-razeen.md`; `docs/session_logs/2026-04-17_1900_sbsa-h2h-uat-six-scenarios.md`; `docs/session_logs/2026-04-17_1310_sbsa-sftp-connectivity-confirmed.md`; `docs/session_logs/2026-04-17_1251_mobilemart-sftp-activation-phase1.md`  
...
### **Latest Achievement (April 21, 2026 - 17:25 SAST)**
**Zapper SFTP user provisioned on sftp-1-vm (v2.99.1)** — Completed MyMoolah-side provisioning for Zapper's daily mark-off SFTP feed. Inserted five rows on the Thorntech gateway VM (`sftp-1-vm` Postgres DB `sftpgw`) mirroring the MobileMart precedent (2026-04-17) row-for-row: `users.id=5 username=zapper user_type=SFTP password=NULL uses_password=false uid=904 gid=904 home_folder_id=4`, `folder.id=4 parent_id=0 name=zapper absolute_path=/zapper cloud_connection_inherited_id=1 cloud_relative_path=/zapper`, `user_folder_permission` READ_WRITE, `authorities` ROLE_SFTP, `public_key.id=3 name=zapper-dillon-2026-04-21 value_len=436 enabled=true generated=false` (Dillon's RSA 2048 key, fingerprint `SHA256:EkGSJ40gqWvwpF+x1m30vB9t5duCBnU9xMUk8GA6Gl4`, cross-verified on local Mac and inside gateway VM before insert — no transit tampering). Firewall rule `allow-zapper-sftp` on `tcp:5022` scoped to Zapper's single static AWS eu-west-1 NAT egress `52.213.37.176/32`. Forward-only migration `20260421_01_fix_zapper_sftp_path.js` corrected `recon_supplier_configs.sftp_path` for `ZAPPER` to `/home/zapper/inbox` on UAT/staging/production (original `20260413_01` was already applied with pre-fix `/home/zapper` value; banking-grade pattern — never edit an applied migration). GCS placeholders at `gs://mymoolah-sftp-inbound/zapper/` and `.../zapper/inbox/`. Runbook: `docs/integrations/ZAPPER_SFTP_PROVISIONING_RUNBOOK.md` (full pre- and post-execution record). Go-live email draft with 1-row synthetic test CSV: `docs/integrations/ZAPPER_EMAIL_DILLON_GO_LIVE.md` + `integrations/zapper/samples/zapper_markoff_TESTHANDSHAKE.csv`. **Gotcha logged**: IAP WebSocket blocked on IPv6-broken Mac (`tunnel.cloudproxy.app` AAAA record, `Errno 51 Network unreachable`); workaround = temporarily add admin IPv4 to `allow-admin-ssh-2222-temp` and SSH directly on 2222. **Second gotcha**: CTE-sibling visibility — `WITH … INSERT … RETURNING` cannot UPDATE a row inserted in a sibling CTE; do the link-home UPDATE as a separate autocommit statement. **Open**: (a) Dillon's test-file upload (smoke test), (b) removal of temporary `102.164.83.33/32` from `allow-admin-ssh-2222-temp` post smoke test. No runtime application code changes. Session log: `docs/session_logs/2026-04-21_1725_zapper-sftp-user-provisioned.md`.

### **Previous Achievement (April 8, 2026 - 09:45)**
