...
**Last Updated**: 2026-04-20 11:44 SAST  
**Latest Feature**: **POL-020 Cash Withdrawal & Own-Funds Ring-Fencing Policy + SBSA cover letter + phased backend implementation plan (v2.99.0)** — Drafted canonical **POL-020 v1.3** (`docs/policies/20-Cash-Withdrawal-Policy.md`) establishing the absolute separation between wallet-holder own funds (ring-fenced, never withdrawable as cash) and third-party funds (the only balance eligible for cash collection via any Cash-Withdrawal Partner — currently Flash eeziCash, EasyPay, Cliquefin/OTT, USSD, and any future equivalent rail). Policy includes count-based velocity controls (§7.6–§7.11): per-tier caps (Tier 1: 2/60m, 3/24h, 15/month; Tier 2: 3/60m, 5/24h, 30/month), FICA-aligned aggregation triggers (R24,999.99 enhanced review; R49,999.99 goAML CTR auto-file), structuring detection (CW-STR-*), channel-rotation detection (CW-CHR-*), OTP step-up at 80% daily cap, pending-review hold at 100%. Thresholds mirrored in **new** `config/cashWithdrawalVelocity.js` and extended `config/kycTierLimits.js` (`getCashWithdrawalCountCaps(tier)`). **POL-004** Transaction Monitoring bumped to v1.3 with new rule family §5.2.8. **T&C v2.4.1** and **FAQ §9c** updated for customer-facing messaging. Phased backend plan authored in `docs/OWN_FUNDS_RINGFENCE_IMPLEMENTATION_PLAN.md` (name-match deposit classifier, single `restricted_balance` pool reuse, Redis-backed rolling counters, `CASH_WITHDRAWAL_VELOCITY_MODE=log_only` default rollout). **SBSA cover letter** (`docs/policies/2026-04-20_StandardBank_CashWithdrawal_Policy_Letter.html`) A4 print-ready, banking-grade, brand-marked with real MyMoolah peaks mark (inlined SVG from `mymoolah-wallet-frontend/assets/logo3.svg`), references PASA TPPP certification (no SBSA sponsorship agreement exists), requests Standard Bank's written no-objection, signed CEO André Botes Sr + CCO André Botes Jr, scrubbed of all "trust account" language in favour of "segregated account" framing. Corpus-wide terminology sweep: 18 policy/handbook docs aligned on "Cash Withdrawal" / "Cash-Withdrawal Partner"; partner-name correction `QuickFin` → **Cliquefin** applied to 25 files (sed pass, zero remaining). Backend code identifiers (`Wallet.canCashOut()`, `cashOutRoutes`, etc.) deliberately preserved. No controllers, services, routes, models, or migrations touched this session. No database changes. No breaking changes. **Letter ships referencing POL-020 v1.1** (explicit André decision — point-in-time snapshot dated 20 April 2026). Prior: SBSA H2H UAT six-scenario test on TEST (2026-04-17, v2.98.1).  
**Document Version**: 2.99.0  
**Session logs**: `docs/session_logs/2026-04-20_1144_sbsa-cash-withdrawal-policy-and-ringfence.md` (this session); prior `docs/session_logs/2026-04-17_1900_sbsa-h2h-uat-six-scenarios.md`; `docs/session_logs/2026-04-17_1310_sbsa-sftp-connectivity-confirmed.md`; `docs/session_logs/2026-04-17_1251_mobilemart-sftp-activation-phase1.md`  
