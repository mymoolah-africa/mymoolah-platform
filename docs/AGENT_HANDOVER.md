...
**Last Updated**: 2026-04-22 11:52 SAST  
**Latest Feature**: **Referral 3-level cleanup + timestamp field-mapping tech debt fix (v2.99.3)** — Two related banking-grade housekeeping items shipped together. (1) Removed the final stale "4-level" references in `models/Referral.js` (JSDoc) and `docs/DEVELOPMENT_GUIDE.md` (3 blocks: commission %, monthly caps claim, schema description). Runtime code, schema, models, services, routes, and frontend were already fully 3-level since migration `20260202_03_referral_3_levels_remove_l4.js`; these were the last stale strings. (2) Applied the proven `OtpVerification` fix pattern to the 13 other Sequelize models that had the same latent `createdAt: 'created_at'` options-only bug: `DisbursementRun`, `KybDocument`, `DisbursementNotificationPreference`, `FlashTransaction`, `Referral`, `DisbursementPayment`, `ReferralPayout`, `DisbursementClientFee`, `SBSAStatementRun`, `ReferralChain`, `DisbursementClient`, `ReferralEarning`, `DisbursementClientUser`. All 14 models now have explicit attribute-level `field: 'created_at'`/`'updated_at'` mappings, preventing future silent `column "X.createdAt" does not exist` failures on revenue-critical paths (referral payouts, disbursement runs). Verified end-to-end against production: 7 representative models successfully ran `where: { createdAt: { [Op.gte]: ... } }` queries returning correct row counts. Production migration status independently confirmed: `level_4_*` columns dropped, CHECK constraint is `BETWEEN 1 AND 3`, 0 rows with `level=4`, `SequelizeMeta` row present. UAT/Staging assumed applied via frontend evidence (proxy tokens were stale); re-verification script saved at `scripts/verify-referral-3level-migration.js`. Zero lint errors.  
**Previous Feature**: **SBSA H2H RM5v2 over-limit re-run on TEST (v2.99.2)** — Scenario 5 re-run with R500,001 tx confirmed 0009 "RUN EXCEEDS LIMIT" gating before 0006 per-tx check.  
**Earlier Feature**: **Zapper SFTP user provisioned on sftp-1-vm (v2.99.1)** — SFTP user, firewall, GCS config, go-live email to Dillon.  
**Document Version**: 2.99.3  
**Session logs**: `docs/session_logs/2026-04-22_1152_referral-3level-cleanup-and-timestamp-mapping-fix.md` (this session); prior `docs/session_logs/2026-04-21_1200_sbsa-h2h-rm5v2-overlimit-rerun.md`; `docs/session_logs/2026-04-20_1500_easypay-test-pins-and-notification-fix.md`  
