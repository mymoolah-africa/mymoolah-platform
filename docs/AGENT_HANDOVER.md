...
**Latest Feature**: **SBSA H2H FSTP finalisation hardening — ✅ IMPLEMENTED / PRODUCTION WRITE APPROVAL REQUIRED** — Investigated André's missing R10 EFT (`0825571055`) and confirmed it was not a wallet-credit bug: production Cloud Run still has `SBSA_SFTP_UPLOAD_ENABLED=false`, `SBSA_STATEMENT_POLLER_MODE=off`, `SBSA_PAIN002_POLLER_MODE=off`, and `RECON_SFTP_WATCHER_MODE=off`; production has no SBSA scheduler jobs; production DB has no `sbsa_statement_runs`, no SBSA deposit rows, and no R10 wallet transaction since 2026-04-20; GCS had no SBSA statement files. Read-only SBSA `/Inbox` listing from `sftp-1-vm` confirmed Penny #2 FINAUD exists and parses cleanly as `ACSP / 0000`, and real MT940/MT942 files contain the R10 as `CR10,00NDEP0825571055`. Fixed `mt940Parser.js` for real SBSA wrapped MT940 and `:34F:` MT942 files, hardened `sbsaStatementService.js` to auto-credit only `DEP` statement credits and use stable bank-line idempotency, and added dry-run-first `scripts/sbsa-h2h-gateway-sync.sh` for VM `/Inbox` → GCS transport. **Do not deploy, run `--apply`, or enable production schedulers without André's explicit approval.**
