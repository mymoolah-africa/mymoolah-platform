...
**Last Updated**: 2026-04-29 10:46 SAST
**Latest Feature**: **EasyPay V5 expiry + partner test PIN fix — ✅ IMPLEMENTED / NEEDS STAGING SEED** — Standardised all active EasyPay PIN/voucher expiry references to **30 days** (`EASYPAY_PIN_EXPIRY_DAYS=30`) across user-facing USSD/SMS copy, frontend cash-out instructions, shared errors, API docs, email drafts, and integration docs. Hardened `scripts/generate-easypay-test-pins.js` so it now requires an explicit `--uat` or `--staging` target and stamps the CSV/XLSX with environment/endpoint columns; for Lesaka testing against `https://staging.mymoolah.africa/billpayment/v1/`, regenerate using `node scripts/generate-easypay-test-pins.js --staging` so `bills` rows exist in the same database the public endpoint uses. EasyPay docs now distinguish local/Codespaces UAT from deployed staging partner testing: staging uses the production EasyPay API credential model via GCP Secret Manager, not local `.env`.
