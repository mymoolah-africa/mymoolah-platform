...
**Last Updated**: 2026-05-13 21:30 SAST
**Latest Feature**: **Support KB — EasyPay V5 cash-in only — ✅ SOURCE + RAG SCOPE / REGENERATE KB** — Removed support copy that described EasyPay as a wallet cash-out: updated `docs/FAQ_MASTER.md` §9b (PEP, retail chains, where to get cash), replaced the EasyPay cash-out GPT gap-fill topic in `scripts/generate-knowledge-base.js`, fixed fee/platform overview seed strings, updated `services/ragService.js` strict scope line, `docs/WITHDRAWALS_COMPLIANCE_AND_KB.md` intents, `docs/CURSOR_2.0_RULES_FINAL.md`, `docs/CHANGELOG.md` (2026-05-13), `docs/AI_SUPPORT_SYSTEM.md` v3.2.1. `npm run check:kb:fresh` passed. **Run approved `generate:kb:update*` / `embed:kb*` per environment** so `ai_knowledge_base` drops stale EasyPay cash-out rows; RAG may also need its ~5-minute KB cache to expire.
