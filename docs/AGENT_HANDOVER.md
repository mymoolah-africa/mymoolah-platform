...
**Last Updated**: 2026-04-24 09:30 SAST  
**Latest Feature**: **SBSA H2H PROD Penny #1 — ✅ SUCCESS (root cause on SBSA side, resolved 2026-04-23 11:54 SAST)** — Full Production H2H round-trip confirmed end-to-end with real money. Yesterday's INCONCLUSIVE verdict at T+30min was caused by a **missing processing rule on SBSA's side** (per Melanie Block, SBSA, email 2026-04-23 18:45 SAST). SBSA fixed the rule at 11:54 SAST; picked up our Pain.001 at ~11:55; delivered **ACK RCVD** at 11:56:47, **INTAUD PDNG / 0000** at 11:56:51, and **FINAUD ACSP / 0000** overnight at 2026-04-24 05:56:06 SAST. Our extended 4-hour poller on `sftp-1-vm` (PID 221588, 120s interval) silently captured the ACK + INTAUD; operator pulled FINAUD manually this morning. **/Outbox is empty** (file fully consumed). **Both statements confirm real-money settlement** on 2026-04-24: R1.00 debit on `272406481` (MyMoolah PROD), R1.00 credit on `10111730633` (Andre Botes personal). Same RCVD→PDNG→ACSP shape as UAT RM7. No change needed on our side. `SBSA_H2H_GO_LIVE=false` still in force in PROD (scheduler pollers, app-level upload, recon watcher all OFF) — will flip only AFTER Penny #2 round-trips cleanly. **Penny #2 scheduled today (2026-04-24, Fri)** at Melanie's invitation, same setup, new MsgId, `ReqdExctnDt=2026-04-27` (Monday, weekend-safe). Added `--exec-date YYYY-MM-DD` CLI override to `scripts/test-sbsa-penny-prod.js`. Evidence: `docs/test/sbsa-sftp-prod-penny-report-2026-04-23.txt` (with RESOLUTION section), `docs/test/sbsa-prod-penny-responses-2026-04-23/received-post-fix/` (ACK+INTAUD+FINAUD XMLs + extended poll log), `docs/test/COLETTE_REPLY_2026-04-24_prod-penny-success.md` (success email + Penny #2 proposal). Session logs below.  
**Previous Feature**: **SBSA H2H PROD Penny #1 live run (2026-04-23, initial verdict INCONCLUSIVE)** — Upload clean but zero responses in 30-min window; escalation sent to Colette+Melanie. Superseded by resolution above.  
**Older Feature**: **SBSA H2H pain002 parser/poller correctness fixes + PROD Penny tooling (v2.99.6)** — Shipped 2026-04-23 (commit `e8005f90`). Deploy gate `SBSA_H2H_GO_LIVE`, AddtlInf capture, response-type classifier, widened filename filter, INTAUD-RJCT terminal logic, UNPAID-authoritative over FINAUD. 23 new unit tests. All 149 disbursement + 26 standardbank tests green.  
**Even Older**: **SBSA H2H RM5v2 over-limit re-run on TEST (v2.99.2)** — Status Code 0009, no FINAUDTST, AddtlInf differentiates 0009 cases. See earlier session logs.  
**Document Version**: 2.99.7  
**Session logs**: `docs/session_logs/2026-04-24_0900_sbsa-penny1-success-and-penny2-plan.md` (this session — Penny #1 RESOLVED, Penny #2 plan); `docs/session_logs/2026-04-23_1132_sbsa-h2h-prod-penny-run.md` (Penny #1 live run, initial INCONCLUSIVE); `docs/session_logs/2026-04-23_1023_sbsa-h2h-prod-penny.md` (v2.99.6 prep); `docs/session_logs/2026-04-21_1200_sbsa-h2h-rm5v2-overlimit-rerun.md`  
