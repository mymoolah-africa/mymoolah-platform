...
**Latest Feature**: **UAT-only wallet availability unfogging — ✅ WALLET BUILD PASSED / STAGING+PRODUCTION PROTECTED** — Added a shared wallet UAT detector and used it to remove fogging only in UAT, including the Codespaces `npm run dev` UAT testing flow where Vite exposes `DEV` and the API URL may be localhost. `mymoolah-wallet-frontend/components/overlays/withdraw-cash/WithdrawCashOverlay.tsx` now treats ABSA and Nedbank cards as available in UAT, including live OTT codes `67` and `4` plus legacy/test codes `112` and `10`; Staging/Production still use API availability as before. `mymoolah-wallet-frontend/components/overlays/shared/AddAccountModal.tsx` enables the MoolahMove tab and international account fields in UAT only; Staging/Production remain disabled with the Coming Soon badge. `mymoolah-wallet-frontend/pages/SendMoneyPage.tsx` removes the MoolahMove fog/Coming Soon pill in UAT only but avoids routing testers into an unsupported legacy beneficiary form. Validation passed: wallet `npx tsc --noEmit`, wallet `npm run build`, and Cursor lints on touched frontend files. Session log: `docs/session_logs/2026-05-14_1525_uat-wallet-unfogging.md`.
