...
**Last Updated**: 2026-04-29 21:50 SAST
**Latest Feature**: **SBSA H2H FSTP finalisation hardening — ✅ IMPLEMENTED / PRODUCTION ACTIVATED WITH APPROVAL** — Investigated André's missing R10 EFT (`0825571055`) and confirmed it was not a wallet-credit bug: production Cloud Run had `SBSA_SFTP_UPLOAD_ENABLED=false`, `SBSA_STATEMENT_POLLER_MODE=off`, `SBSA_PAIN002_POLLER_MODE=off`, and `RECON_SFTP_WATCHER_MODE=off`; production had no SBSA scheduler jobs; production DB had no `sbsa_statement_runs`, no SBSA deposit rows, and no R10 wallet transaction since 2026-04-20; GCS had no SBSA statement files. Read-only SBSA `/Inbox` listing from `sftp-1-vm` confirmed Penny #2 FINAUD exists and parses cleanly as `ACSP / 0000`, and real MT940/MT942 files contain the R10 as `CR10,00NDEP0825571055`. Fixed `mt940Parser.js` for real SBSA wrapped MT940 and `:34F:` MT942 files, hardened `sbsaStatementService.js` to auto-credit only `DEP` statement credits and use stable bank-line idempotency, added dry-run-first `scripts/sbsa-h2h-gateway-sync.sh` for VM `/Inbox` → GCS transport, fixed real production statement filename matching, and removed the Cloud Run `scripts/db-connection-helper.js` runtime dependency from `notificationEngine.js` so Pain.002 can load in the Docker image. André approved commit → push → redeploy → process. Production revision `mymoolah-backend-production-00156-nqw` credited the R10 to user 1 at 2026-04-29 20:32 UTC: wallet `WAL-1775063499170-1` balance moved to `18.38`, `standard_bank_transactions.id=14`, `transactions.id=56`, and balanced journal `SBSA-DEP-STMT-204c03f52fbafa78d45bf8821b30506a` posted debit `1100-01-01` / credit `2100-01-01` for R10. Continue syncing SBSA `/Inbox` in limited batches until the gateway service is scheduled.
...
**Session logs**: `docs/session_logs/2026-04-29_2150_sbsa-h2h-fstp-finalisation.md` (latest — SBSA H2H FSTP parser/gateway hardening); `docs/session_logs/2026-04-29_2020_easypay-reference-migration-hardening.md`; `docs/session_logs/2026-04-29_1904_ott-payout-scaffold.md`; `docs/session_logs/2026-04-29_1046_easypay-v5-expiry-staging-test-pins.md`; `docs/session_logs/2026-04-28_1745_ott-mobile-integration-framework.md`
