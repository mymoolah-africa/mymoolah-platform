...
**Last Updated**: 2026-04-20 15:30 SAST  
**Latest Feature**: **EasyPay UAT test PINs generated + notification fix (v2.98.2)** — Generated `easypay_test_pins.csv` (50 rows, 45 DB bills + 5 invalid PINs) and sent to Razeen with UAT SessionToken via onetimesecret.com. Fixed `easyPayController.js`: replaced SMS with in-app notification engine for deposit confirmations. Added PIN expiry alignment question (A5) to Q&A checklist. EasyPay scheduling UAT sprint.  
**Previous Feature**: **SBSA H2H UAT six-scenario test executed on TEST (v2.98.1)** — Ran Colette's six UAT scenarios back-to-back against SBSA TEST (196.8.85.62:5022), 20-min intervals, 15:55 → 18:36 SAST. All six uploads accepted by /Outbox/ within 2s. Verdicts: **RM7 Valid SSVS — PASS** (ACK RCVD, INTAUD PDNG×3, FINAUD ACSP×3 in 3m40s); **RM8 Duplicate MsgId — PASS** (NACK RJCT in 17s, AddtlInf "Duplicate MsgId"); **RM9 Invalid DbtrAcct — PASS*** (ACK RCVD, INTAUD RJCT×3 Status Code **0009** — test sheet expected 0003, SBSA to confirm canonical code); **RM10 Past execution date — PASS exact match** (ACK RCVD, INTAUD RJCT×3 Status Code 0014); **RM11 Over limit R96.15 — PARTIAL** (R96.15 below TEST profile limit, over-limit path not triggered — all 3 tx ACSP); **RM12 10-tx mixed — PARTIAL** (ACK + INTAUD PART 4P+6R + FINAUD PART 4A+6R + UNPAID PART 2tx all received; VET for this MsgId not received in polling window). 19 response XMLs captured in `docs/test/sbsa-uat-responses-2026-04-17/responses/`. Formal report: `docs/test/sbsa-sftp-uat-report-2026-04-17.txt`. Draft email to Colette: `docs/test/COLETTE_UAT_REPLY_2026-04-17.md` (prose only). New test harness: `scripts/test-sbsa-h2h-scenarios.js`. **Key finding**: UNPAID response can override FINAUD on a per-transaction basis — RM12 Tx-03 ACWC reason 14 and Tx-04 RJCT reason 03 had ACSP in FINAUD minutes earlier. pain002 watcher must treat UNPAID as authoritative over FINAUD. Earlier today: SBSA H2H SFTP end-to-end round-trip closed on TEST after `MYMOOLAH` vs `mymoolahuser` username fix (first success on PROD auth too). Prior: MobileMart SFTP activation Phase 1; SBSA H2H SFTP hardening (v2.98.0).  
**Document Version**: 2.98.1  
**Session logs**: `docs/session_logs/2026-04-17_1900_sbsa-h2h-uat-six-scenarios.md` (this session); prior `docs/session_logs/2026-04-17_1310_sbsa-sftp-connectivity-confirmed.md`; `docs/session_logs/2026-04-17_1251_mobilemart-sftp-activation-phase1.md`; `docs/session_logs/2026-04-17_1054_sbsa-h2h-sftp-hardening.md`  
