...
**Latest Feature**: **SBSA H2H SFTP end-to-end round-trip closed on TEST (v2.98.1)** — Resolved 2026-04-16/17 authentication failures (root cause: agent used `MYMOOLAH` filename prefix as SFTP login instead of documented `mymoolahuser`). With the correct username, public-key auth **passes on both SBSA TEST (`196.8.85.62:5022`) and PROD (`196.8.86.53:5022`)** — PROD was a first-ever successful auth since SBSA imported our key. Uploaded `MYMOOLAH_OWN11_Pain001v3_ZAR_TST_20260417110153RM1.xml` (4,226 bytes) to TEST `/Outbox/`; SBSA collected file within 45 seconds. **Full response cycle closed in 17 minutes**: ACK (pain.002 GrpSts RCVD) at 13:14 SAST, INTAUDTST (3 tx PDNG) at 13:14, FINAUDTST (3 tx ACSP — Accepted for Settlement) at 13:18. SBSA uses dual-delivery: responses land in both `/BAS/` (appended `_ACK_/_INTAUDTST_/_FINAUDTST_` naming) and `/Inbox/` (ISO-style `MYMOOLAH_OWN11_ACK_TST_*` naming). All 6 response XMLs captured in `docs/test/sbsa-responses-2026-04-17/`. High-risk tech-debt rule added: agents must always cross-reference the relevant setup doc before running partner-facing live tests. New communication rule: no markdown tables in email drafts. Prior: MobileMart SFTP activation Phase 1; SBSA H2H SFTP hardening (v2.98.0).  
**Document Version**: 2.98.1  
**Session logs**: `docs/session_logs/2026-04-17_1310_sbsa-sftp-connectivity-confirmed.md` (this session); prior `docs/session_logs/2026-04-17_1251_mobilemart-sftp-activation-phase1.md`; `docs/session_logs/2026-04-17_1054_sbsa-h2h-sftp-hardening.md`  
