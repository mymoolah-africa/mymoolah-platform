...
**Latest Feature**: **VAT strategy formalisation and pass-through correction — ✅ IMPLEMENTED / PRODUCTION CORRECTIONS APPLIED** — Canonical policy added in `docs/VAT_ACCOUNTING_STRATEGY.md`: MMTP records VAT control/TaxTransaction rows only for MMTP-owned revenue, markup, and commission. Supplier/bank/client/merchant throughput fees are VAT-inclusive pass-through amounts posted to clearing/payable accounts. Forward code aligned for PayShap RPP/RTP, Zapper QR, EasyPay cash-out, and Flash cash-out. Production historical cleanup: RPP was already corrected with `CORR-RPP-PASS-*`; RTP correction script `scripts/correct-production-rtp-pass-through-ledger.js` applied 6 `CORR-RTP-PASS-*` journals, reclassifying R34.50 from SBSA cost/VAT control to supplier clearing. Final read-only checks found no remaining Zapper VAT debit lines, EasyPay provider-expense lines, or Flash cash-out VAT-control lines matching legacy pass-through patterns.  
