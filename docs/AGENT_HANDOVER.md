...
**Latest Feature**: **SBSA H2H SFTP connectivity confirmed on TEST and PROD (v2.98.1)** — Resolved 2026-04-16/17 authentication failures. Root cause was operational: earlier test commands used `MYMOOLAH` (filename prefix / ISO-20022 party ID) as the SFTP login username instead of the documented `mymoolahuser`. With the correct username, public-key auth **passes on both SBSA TEST (`196.8.85.62:5022`) and PROD (`196.8.86.53:5022`)** — PROD was a first-ever successful auth since SBSA imported our key. Uploaded `MYMOOLAH_OWN11_Pain001v3_ZAR_TST_20260417110153RM1.xml` (4,226 bytes) to TEST `/Outbox/`; SBSA collected the file within 45 seconds. ACK/NACK in `/BAS/` pending SBSA processing queue. Clean test report and reply email to Colette drafted. High-risk tech-debt rule added: agents must always cross-reference the relevant setup doc before running partner-facing live tests. Prior: MobileMart SFTP activation Phase 1; SBSA H2H SFTP hardening (v2.98.0).  
