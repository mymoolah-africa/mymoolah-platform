...
**Last Updated**: 2026-04-27 20:24 SAST
**Latest Feature**: **Cursor rules optimization — ✅ IMPLEMENTED / LOWER CONTEXT LOAD** — `.cursor/rules/*.mdc` was reviewed and rewritten for accuracy, efficiency, and lower always-on token load. All rule files now validate under 50 lines, stale/duplicated wording was removed, `tech-debt.mdc` was slimmed into a documentation discipline rule, `skill-routing.mdc` was added so agents load the right `.agents/skills/` file only when needed, and `frontend-standards.mdc` was added as a file-scoped rule for wallet/portal frontend work. Current project law remains explicit: code-first frontend, Figma historical only, DB-helper-only direct DB access, approved Codespaces restart scripts, safe git behavior, PII redaction, and approval before destructive/production actions.
**Previous Feature**: **Figma retired from active frontend workflow — ✅ IMPLEMENTED / CODE-FIRST POSITIONING** — Active code and docs were swept so Figma is positioned correctly as historical only. MyMoolah frontend work is now explicitly code-first: React/TypeScript/Tailwind files and the MyMoolah design system are the source of truth. `ImageWithFallback` was moved from `components/figma/` to `components/common/`, imports were updated, obsolete Figma integration scripts/docs were removed or replaced with historical notes, and active docs/rules now state that Figma is not a current design platform, source of truth, approval gate, or read-only constraint. Archived/session-log references remain historical records only.
**Older Feature**: **Agent skills accuracy and performance upgrade — ✅ IMPLEMENTED / DOCS-ONLY** — `.agents/skills/` was reviewed and upgraded for accuracy, lower token load, and fewer stale instructions. The two oversized skills now load quickly: `auditing` v2.2.0 is a concise ledger/VAT/reconciliation router with the old full content preserved in `auditing/reference-full.md`; `admin-portal-builder` v1.1.0 is a concise portal/RBAC/maker-checker guide with the old full content preserved in `admin-portal-builder/reference-full.md`. Correctness drift was fixed across active skills: JWT HS512, code-as-frontend-source-of-truth, MyMoolah brand green `#86BE41` + blue `#2D8CCA` + Montserrat, DB-helper-only direct scripts, safe concurrent-index migration example, Cloud Scheduler-first scheduled jobs, single-Redis lock wording, and visual skill routing via `_shared/visual-output-reference.md`. No runtime app code or database records changed.
...
**Session logs**: `docs/session_logs/2026-04-27_2024_cursor-rules-optimization.md` (this session — Cursor rules optimization); `docs/session_logs/2026-04-27_2012_figma-retired-code-first-frontend.md`; `docs/session_logs/2026-04-27_1952_agent-skills-upgrade.md`; `docs/session_logs/2026-04-27_1530_production-audit-txn33-ledger-correction.md`; `docs/session_logs/2026-04-26_1610_vat-pass-through-strategy.md`
