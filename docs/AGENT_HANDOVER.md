...
**Last Updated**: 2026-05-10 21:19 SAST
**Latest Feature**: **Voucher / Gift Card split — ✅ IMPLEMENTED / BUILD PASSED** — Investigated André's screenshots showing overlap between `/vouchers-overlay` and `/gift-cards-overlay`. Confirmed the routes intentionally share `DigitalVouchersOverlay` and the existing voucher catalog/purchase engine, but retail mode was showing the full catalog while gift-card mode showed the `isGiftCard` subset, causing gift-card brands to appear on both pages. Updated `mymoolah-wallet-frontend/components/overlays/digital-vouchers/DigitalVouchersOverlay.tsx` so `Gift Cards` shows only `isGiftCard === true` and `Buy Retail Vouchers` excludes those products with `isGiftCard !== true`. Also fixed the gift-card loading failure copy so it no longer says retail vouchers, and tightened local catalog item typing to keep targeted ESLint clean. Validation passed: `npx jest tests/voucherCatalogBrandService.test.js --runInBand` 39/39, targeted ESLint on `DigitalVouchersOverlay.tsx`, wallet `npm run build`, and Cursor lints on the edited overlay. No backend purchase logic, database schema, ledger, supplier integration, or production data changed. Session log: `docs/session_logs/2026-05-10_2119_voucher-gift-split.md`.
