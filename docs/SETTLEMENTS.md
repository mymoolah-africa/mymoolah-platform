...
**Wallet cash withdrawals (Apr 2026):** Outbound consumer flows include **partner-facilitated cash** through **Cash-Withdrawal Partners** (currently eeziCash via Flash Group — float `1200-10-04`; EasyPay retail cash-withdrawal — float `1200-10-03` under its historical "EasyPay Cash-out Float" ledger name; Cliquefin / OTT cash-withdrawal references on onboarding; USSD cash-withdrawal) alongside electronic rails (EFT, PayShap). Every such cash withdrawal is economically and regulatorily a **wallet debit → cash collection** path under the TPPP framework — not VAS retail-voucher resale, and (per POL-020) fundable only from Third-Party Funds. See `docs/policies/20-Cash-Withdrawal-Policy.md`, `docs/WITHDRAWALS_COMPLIANCE_AND_KB.md` and `docs/integrations/MyMoolah_TPPP_Withdrawal_Flow_Diagrams.html` for narrative and ledger excerpts.
