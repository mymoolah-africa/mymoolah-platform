...
| C2 | Does EasyPay provide a **daily settlement / reconciliation file** (CSV, SFTP, API)? If yes: **format**, **delivery channel**, **cut-off timezone**, and **field mapping** (e.g. EP txn id ↔ `Reference` in V5). | Implement automated recon (pending today). |
...
| D2 | **IP allowlisting**: definitive EasyPay **egress CIDRs** per environment for MMTP load balancer / Cloud Run. | Production hardening. |
